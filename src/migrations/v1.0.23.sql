-- v1.0.23 - Harden setup bootstrap detection by installation.
...
BEGIN
  IF EXISTS (
    ...
    VALUES ('1.0.23', 'Harden setup bootstrap detection by installation', true, 0)
...
BEGIN
  ...
  IF EXISTS (
    ...
    SET db_version = '1.0.23', updated_at = timezone('utc'::text, now())
    WHERE id = target_id;
  ELSE
    UPDATE public.system_info
    SET db_version = '1.0.23'
    WHERE id = target_id;
  END IF;
END $$;
