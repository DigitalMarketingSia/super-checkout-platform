...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    const sendOrderEmail = async (
        ...
        try {
            const recipientEmail = recipient?.email || order.customer_email;
            const recipientName = recipient?.name || order.customer_name || 'Cliente';

            if (!recipientEmail) {
                console.warn(`[Webhook] No recipient email available for Order ${order.id}. Skipping email.`);
                return;
            }
    ...
    // Helper to get raw body from stream
        ...
        if (supabaseKey) {
            try {
                ...
                if (gateways && gateways.length > 0) {
                    ...
                    accessToken = decrypt(gateway.private_key);
