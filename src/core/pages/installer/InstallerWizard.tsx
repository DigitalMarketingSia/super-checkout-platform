...
export default function InstallerWizard() {
    ...
    // Manual self-hosted mode: values remain only in this browser step.
    ...
    useEffect(() => {
        const validateInstallationToken = async () => {
            ...
            const devBypass = import.meta.env.DEV && params.get('bypass') === 'true';
            ...
            if (devBypass) {
                ...
                try {
                    ...
                    console.error('[Installer] Bypass auth failed:', safeInstallerErrorMessage(e));
                    setAccessDenied(true);
            ...
            try {
                ...
                const message = safeInstallerErrorMessage(err);
                console.error('[Installer] Token validation failed:', message);
                setAccessError(message || t('errors.invalid_token'));
                setAccessDenied(true);
                setIsValidatingToken(false);
            }
        };
    ...
    const consumeInstallToken = async (domain: string, targetInstallationId = installationId) => {
        if (!installToken) {
        ...
        if (!response.ok || !data.valid || !data.consumed) {
            throw new Error(data.message || t('errors.invalid_token'));
        }
        ...
        try {
            ...
            if (!response.ok) {
                ...
                try {
                    ...
                    console.error('[Installer] Non-JSON license response:', safeInstallerErrorMessage(e));
                }
                    ...
                    {currentStep === 'supabase_keys' && (
                        <div className="glass-panel border border-white/10 bg-white/5 backdrop-blur-xl rounded-2xl p-8 shadow-2xl animate-in fade-in slide-in-from-bottom-4">
                            ...
                            <form onSubmit={async (e) => {
                                ...
                                if (!anonKey.trim() || !manualServiceRoleKey.trim()) {
                                ...
                                try {
                                    ...
                                    const { error: schemaProbeError } = await tempClient
                                        .from('domains')
                                        .select('id')
                                        .limit(0);
                                    if (schemaProbeError) {
                                        console.warn('[Installer] Schema cache probe failed:', safeInstallerErrorMessage(schemaProbeError));
                                    }
                                    console.log('✅ Schema cache probe completed');
                                    ...
                                    while (retryCount < 2) {
                                        try {
                                            ...
                                            console.log('[Installer] License validation response:', {
                                                status: validationRes.status,
                                                valid: !!validationData?.valid,
                                                has_license: !!validationData?.license
                                            });

                                            if (!validationRes.ok || !validationData.valid) {
                                                const msg = safeInstallerErrorMessage(validationData.message || 'Licença inválida ou bloqueada pelo servidor central.');
                                            ...
                                            console.log('[Installer] License validated:', {
                                                valid: true,
                                                plan: validationData?.license?.plan || null,
                                                has_owner_id: !!validationData?.license?.owner_id
                                            });
                                            break; // Success

                                        } catch (valError: any) {
                                            const validationErrorMessage = safeInstallerErrorMessage(valError);
                                            // Pass through if it's the final attempt
                                            if (retryCount >= 1 || (!validationErrorMessage.includes('duplicate') && !validationErrorMessage.includes('installations_installation_id_key'))) {
                                                console.error('[Installer] Validation failed:', validationErrorMessage);
                                                alert(`Falha na validação da licença: ${validationErrorMessage}`);
                                                setLoading(false);
                                                return;
                                            }
                                        }
                                    ...
                                    if (licenseError) {
                                        if (!licenseError.message.includes('duplicate')) {
                                            console.warn('License injection warning:', safeInstallerErrorMessage(licenseError));
                                        } else {
                                    ...
                                    try {
                                        ...
                                        if (domainError && !domainError.message.includes('duplicate')) {
                                            console.warn('Failed to register installation domain:', safeInstallerErrorMessage(domainError));
                                        } else {
                                            console.log('✅ Installation domain registered in database.');
                                        }
                                    } catch (domainErr) {
                                        console.warn('Error registering installation domain:', safeInstallerErrorMessage(domainErr));
                                    }

                                    // 4.2 SAVE INSTALLATION ID TO DATABASE (CRITICAL FIX)
                                    // This ensures LicenseGuard can retrieve the installation_id
                                    try {
                                        console.log('[Installer] Saving installation ID to database:', {
                                            has_installation_id: !!currentInstallId
                                        });
                                        ...
                                        if (configError && !configError.message.includes('duplicate')) {
                                            console.warn('Failed to save installation_id:', safeInstallerErrorMessage(configError));
                                        } else {
                                        ...
                                        console.warn('Error saving installation config:', safeInstallerErrorMessage(configErr));
                                    }
                                    ...
                                    try {
                                        ...
                                        if (installationError) {
                                            console.warn('Failed to register local installation:', safeInstallerErrorMessage(installationError));
                                        } else {
                                ...
                                <div>
                                    ...
                                    <input type="password" value={manualServiceRoleKey} onChange={e => setManualServiceRoleKey(e.target.value)} required autoComplete="off"
                                        className="w-full bg-black/40 border border-white/10 rounded-xl px-4 py-3 text-white text-xs font-mono" />
                                    <p className="mt-2 text-xs text-amber-200/80">{t('keys.manual_secret_notice')}</p>
                                </div>
                    ...
                    {currentStep === 'deploy' && (
                        <div className="glass-panel border border-white/10 bg-white/5 backdrop-blur-xl rounded-2xl p-8 shadow-2xl animate-in fade-in slide-in-from-bottom-4">
                            ...
                            <div className="space-y-6">
                                <div className="bg-black/40 rounded-xl p-6 border border-white/10">
                                    ...
                                    <div className="mb-6 rounded-xl border border-amber-400/30 bg-amber-400/10 p-4 text-sm text-amber-100">
                                        <div className="mb-1 flex items-center gap-2 font-bold">
                                            ...
                                            {t('deploy.server_only_title')}
                                        </div>
                                        <p className="text-amber-100/80">{t('deploy.server_only_desc')}</p>
                                    </div>
                                ...
                                <button onClick={() => {
                                    clearEphemeralManualSecrets();
                                    setCurrentStep('vercel_config');
                                }} className="w-full bg-[#3ECF8E] hover:bg-[#3ECF8E]/90 text-black font-bold py-3 rounded-xl flex items-center justify-center gap-2">
                                    {t('deploy.secrets_configured_continue')} <ChevronRight className="w-4 h-4" />
                                </button>
                            </div>
                        </div>
                    )}
