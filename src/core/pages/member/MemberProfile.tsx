...
const getGrantName = (grant: AccessGrant) => {
    if (grant.product_id) return grant.product?.name || '';
    return grant.content?.title || '';
};
...
export const MemberProfile = () => {
        ...
        accessGrants
            ...
            .forEach((grant) => {
                if (grant.product_id) {
                    ...
                    if (!existing || new Date(grant.granted_at) > new Date(existing.granted_at)) {
                        productGrants.set(grant.product_id, {
                            ...grant,
                            displayType: 'product',
                            displayName: getGrantName(grant) || t('profile.product_subscription', 'Produto / Assinatura'),
                        });
                    }
                    return;
                }
                ...
                contentGrants.push({
                    ...grant,
                    displayType: 'content',
                    displayName: getGrantName(grant) || t('profile.single_content', 'Conteúdo avulso'),
                });
            });
    ...
    return (
        <div className="max-w-5xl mx-auto px-4 py-8">
            <div className="mb-8">
                <p className="text-xs font-bold uppercase tracking-widest text-gray-500 mb-2">{memberArea?.name || t('layout.member_area_fallback', 'Área de membros')}</p>
