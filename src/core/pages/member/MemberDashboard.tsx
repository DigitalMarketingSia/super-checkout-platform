    ...
    const handleItemClick = async (item: any) => {
        ...
        if (item.product) {
            ...
            try {
                ...
                alert(t('dashboard.alerts.product_content_error', 'Erro ao carregar conteúdo do produto.'));
            }
            return;
        }
        ...
        handleAccess(item, {
            onAccess: () => {
                ...
                if (item.content) {
                    ...
                    if (contentId) {
                        ...
                        alert(t('dashboard.alerts.lesson_content_not_found', 'Erro ao navegar: conteúdo da aula não encontrado.'));
                    }
                }
            },
            onSalesModal: (product) => {
                ...
                if (product) {
                    ...
                    alert(t('dashboard.alerts.exclusive_without_product', 'Este conteúdo é exclusivo para assinantes e não possui um produto direto associado para compra.'));
                }
            }
        });
    };
                            ...
                            {memberArea?.banner_button_text && memberArea?.banner_button_link ? (
                                ...
                                <>
                                    <button
                                        ...
                                        style={{ backgroundColor: memberArea?.primary_color, color: '#fff' }}
                                    >
                                        <Play className="w-5 h-5 fill-current" /> {t('dashboard.watch', 'Assistir')}
                                    </button>
                                    <button className="flex items-center gap-2 bg-gray-500/70 text-white px-8 py-3 rounded font-bold hover:bg-gray-500/90 transition-colors backdrop-blur-sm">
                                        <Info className="w-5 h-5" /> {t('dashboard.more_info', 'Mais informações')}
                                    </button>
                                </>
                            ) : null}
                        </div>
            ...
            <div className="px-4 md:px-24 lg:px-32 space-y-12 relative z-10 -mt-10 pb-20">
                {tracks.length === 0 ? (
                    <div className="text-gray-500 py-10 text-center border border-white/10 rounded-xl bg-white/5">
                        {memberArea ? t('dashboard.empty_tracks', 'Nenhuma trilha configurada.') : t('dashboard.select_member_area', 'Selecione uma área de membros.')}
                    </div>
                ) : (
                    tracks.map(track => (
                        <TrackSlider
