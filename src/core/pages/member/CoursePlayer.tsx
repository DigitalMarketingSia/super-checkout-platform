...
import { ChevronLeft, CheckCircle, Circle, FileText, Download, ChevronDown, ChevronUp, PanelLeftClose, Search, Play, ChevronRight, Lock, Menu } from 'lucide-react';
...
export const CoursePlayer = ({ forcedSlug }: { forcedSlug?: string } = {}) => {
    ...
    const loadData = async (contentId: string) => {
        ...
        try {
            ...
            if (!currentMemberArea && isCustomDomain) {
                throw new Error(t('course.errors.member_area_not_found', 'ÃƒÂrea de membros nÃƒÂ£o encontrada para este domÃƒÂ­nio.'));
            }
            ...
            if (!targetContent) {
                ...
                throw new Error(t('course.errors.content_not_found', 'ConteÃƒÂºdo nÃƒÂ£o encontrado.'));
            }
            ...
            setError(error instanceof Error ? error.message : t('course.errors.load_course', 'Erro ao carregar curso.'));
        } finally {
            setLoading(false);
        }
    };
    ...
    const renderContent = () => {
        if (!currentLesson) return <div className="text-white">{t('course.select_lesson', 'Selecione uma aula')}</div>;
                    ...
                    return currentLesson.file_url ? (
                        <div key="file" className="bg-white/5 rounded-xl p-6 border border-white/5 flex items-center justify-between">
                            <div className="flex items-center gap-4">
                                ...
                                <div>
                                    <h3 className="font-bold text-white text-sm">{t('course.supplementary_material', 'Material complementar')}</h3>
                                    <p className="text-xs text-gray-400">{t('course.file_access_hint', 'Clique para acessar o arquivo ou link externo')}</p>
                                </div>
                            </div>
                            <a
                            ...
                            >
                                <Download className="w-4 h-4" /> {t('course.access_resource', 'Acessar recurso')}
                            </a>
                        </div>
                    ) : null;
                ...
                case 'gallery':
                    return (currentLesson.gallery && currentLesson.gallery.length > 0) ? (
                        <div key="gallery" className="pt-8 border-t border-white/5">
                            <h3 className="text-xl font-bold text-white mb-6">{t('course.resource_gallery', 'Galeria de recursos')}</h3>
            ...
            <div className="LESSON-CONTAINER w-full max-w-[1100px] mx-auto px-6 space-y-8 pb-20">
                ...
                <div className="flex flex-col md:flex-row items-center justify-between gap-6 pt-8 border-t border-white/10 pb-8">
                    <div className="w-full md:w-auto">
                        ...
                        <p className="text-gray-400 text-sm">{t('course.module_label', 'MÃƒÂ³dulo')}: {modules.find(m => m.id === currentLesson.module_id)?.title}</p>
                    </div>
                    ...
                    <div className="flex items-center gap-3 w-full md:w-auto justify-between md:justify-end">
                        ...
                        >
                            {progressMap[currentLesson.id] ? (
                                <>
                                    <CheckCircle className="w-5 h-5" /> {t('course.completed', 'ConcluÃƒÂ­da')}
                                </>
                            ) : (
                                <>
                                    <Circle className="w-5 h-5" /> {t('course.mark_completed', 'Marcar como concluÃƒÂ­da')}
                                </>
                            )}
                        </button>
                    </div>
                        ...
                        <div className="p-4 flex items-center justify-between gap-2 sticky top-0 bg-[#0f131a]/95 backdrop-blur-sm z-20 border-b border-white/5">
                            <div className="relative flex-1">
                                ...
                                <input
                                    id="search-input"
                                    type="text"
                                    placeholder={t('course.search_placeholder', 'Buscar conteÃƒÂºdo')}
                                    value={searchTerm}
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar">
                            {filteredContents.map((c) => {
                                ...
                                const displayTitle = isLockedContent
                                    ...
                                    : c.title;
                                const secondaryTitle = isLockedContent && c.associated_product?.name && c.associated_product.name !== c.title
                                    ? c.title
                                    : null;
                                        ...
                                        >
                                            <div className="flex items-center gap-3 min-w-0">
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    <h3 className={`text-sm font-bold truncate leading-tight ${isCurrentContent ? 'text-white' : isLockedContent ? 'text-white' : 'text-gray-400'}`}
                ...
                <div className="LESSON-WRAP flex-1 overflow-y-auto bg-[#0D1118] p-4 md:p-8 pt-20 md:pt-16 flex justify-center">
                    {loading ? (
                        ...
                        <div className="flex items-center justify-center h-64 w-full text-center">
                            <div>
                                <p className="text-red-500 font-bold mb-2">{t('course.error_title', 'Erro')}</p>
