    ...
    if (loading) {
        return (
            <div className="py-32 flex flex-col items-center justify-center">
                ...
                <p className="text-white/40 text-[10px] font-bold uppercase tracking-widest font-mono italic">{t('member_area_products.loading')}</p>
            </div>
        );
    }
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 max-w-7xl mx-auto">
            ...
            >
                ...
                <div className="flex items-center gap-5 relative z-10">
                    <div>
                        <h2 className="text-xl font-black text-white italic uppercase tracking-tighter leading-none mb-1">
                            {t('member_area_products.header.title_prefix')}{' '}
                            <span style={{ color: primaryColor }}>{t('member_area_products.header.title_highlight')}</span>
                        </h2>
                        <div className="flex items-center gap-4">
                            <div className="flex items-center gap-2 text-white/40 text-[10px] font-bold uppercase tracking-[0.3em]">
                                <Database className="w-3.5 h-3.5" />
                                {t('member_area_products.header.pricing_nodes')}
                            </div>
                            ...
                            <div className="flex items-center gap-2 text-white/60 text-[10px] font-mono uppercase tracking-[0.2em]">
                                ...
                                {t('member_area_products.header.linked_offers', { count: products.length })}
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="hidden lg:flex flex-col items-end relative z-10">
                    <span className="text-[10px] font-black text-white/20 uppercase tracking-[0.4em] leading-none mb-2">{t('member_area_products.header.coverage')}</span>
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-xs font-mono text-white/60">{t('member_area_products.header.active')}</span>
                    </div>
                </div>
            </div>
            ...
            {products.length === 0 ? (
                <div className="py-32 flex flex-col items-center gap-8 bg-black/20 border border-white/5 border-dashed rounded-[4rem]">
                    ...
                    <div className="text-center space-y-2">
                        <p className="text-sm font-black text-white italic uppercase tracking-[0.2em]">{t('member_area_products.empty.title')}</p>
                        <p className="text-[10px] font-mono text-white/20 uppercase tracking-widest">{t('member_area_products.empty.desc')}</p>
                    </div>
                </div>
            ) : (
                                    ...
                                    <div className="min-w-0">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <span className={`px-3 py-1 rounded-full text-[8px] font-black uppercase tracking-widest border ${
                                                ...
                                                {isLinked ? t('member_area_products.card.synced') : t('member_area_products.card.offline')}
                                            </span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-6 relative z-10">
                                    <div className="flex flex-col gap-2">
                                        <div className="flex justify-between items-center px-1">
                                            <span className="text-[10px] font-black text-white/20 uppercase tracking-widest">{t('member_area_products.card.coverage')}</span>
                                            ...
                                            >
                                                {processingId === product.id ? <RefreshCw className="w-4 h-4 animate-spin mx-auto" /> : (
                                                    <div className="flex items-center justify-center gap-2">
                                                        ...
                                                        <span>{t('member_area_products.card.revoke_access')}</span>
                                                    </div>
                                                )}
                                            </button>
                                        ) : (
                                            ...
                                            >
                                                {processingId === product.id ? <RefreshCw className="w-4 h-4 animate-spin mx-auto" /> : (
                                                    <div className="flex items-center justify-center gap-2">
                                                        ...
                                                        <span className="font-black uppercase italic tracking-widest">{t('member_area_products.card.sync_all')}</span>
                                                    </div>
                                                )}
                                            </button>
                                        )}
