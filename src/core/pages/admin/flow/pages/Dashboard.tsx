...
export const Dashboard = () => {
  ...
  const fetchFunnels = async () => {
    try {
      ...
      toast.error(t('flow.dashboard.toasts.load_error', 'Erro ao carregar seus projetos.'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreateNew = async () => {
    try {
      ...
      const newFunnel = {
        ...
        name: t('flow.dashboard.new_funnel_name', 'Novo Funil - {{date}}', { date: new Date().toLocaleDateString() }),
      ...
      if (error) throw error;
      if (data) {
        toast.success(t('flow.dashboard.toasts.created', 'Projeto criado com sucesso!'));
        navigate(`/admin/flow/editor/${data.id}`);
      }
    } catch (error) {
      console.error('Erro ao criar funil:', error);
      toast.error(t('flow.dashboard.toasts.create_error', 'Não foi possível criar um novo projeto.'));
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (error) throw error;
      toast.success(t('flow.dashboard.toasts.deleted', 'Projeto excluído com sucesso.'));
      setFunnels(funnels.filter(f => f.id !== funnelToDelete));
    } catch (error) {
      console.error('Erro ao excluir:', error);
      toast.error(t('flow.dashboard.toasts.delete_error', 'Erro ao excluir o projeto.'));
    } finally {
  ...
  const handleEditSubmit = async () => {
    ...
    if (!editName.trim()) {
      toast.error(t('flow.dashboard.toasts.name_required', 'O nome do projeto é obrigatório.'));
      return;
    }
    ...
    try {
      ...
      if (error) throw error;
      toast.success(t('flow.dashboard.toasts.updated', 'Projeto atualizado com sucesso.'));
      ...
      toast.error(t('flow.dashboard.toasts.update_error', 'Erro ao atualizar o projeto.'));
    } finally {
      setSavingEdit(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-[#05050A] text-white p-4 lg:p-12">
      <div className="max-w-[1400px] mx-auto space-y-12">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 animate-in fade-in slide-in-from-top-8 duration-700">
          <div className="space-y-4">
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-black uppercase tracking-[0.4em] text-cyan-500 leading-none italic">{t('flow.dashboard.badge', 'Flow Builder')}</span>
            ...
            <h1 className="font-portal-display italic font-black text-4xl md:text-6xl text-white uppercase tracking-tighter leading-none">
              {t('flow.dashboard.title_prefix', 'Meus')} <span className="bg-gradient-to-r from-[#27CBEF] to-[#27CBEF]/60 bg-clip-text text-transparent">{t('flow.dashboard.title_highlight', 'Funis')}</span>
            </h1>
            <p className="text-gray-400 text-lg font-medium leading-relaxed max-w-xl">
              {t('flow.dashboard.subtitle', 'Gerencie seus projetos e automações visuais com tecnologia de ponta.')}
            </p>
          </div>
          ...
          >
            ...
            {t('flow.dashboard.new_project', 'Novo projeto')}
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-32 bg-[#0F0F13] border border-white/5 rounded-[2.5rem] animate-in fade-in duration-1000">
            ...
            <h2 className="text-3xl font-portal-display text-white mb-4 italic tracking-tighter">{t('flow.dashboard.empty_title', 'Nada por aqui ainda')}</h2>
            <p className="text-gray-500 text-center max-w-md mb-10 font-medium">
              {t('flow.dashboard.empty_desc', 'Comece agora a desenhar seu primeiro funil visual e escale suas automações.')}
            </p>
            <button 
              onClick={handleCreateNew}
            ...
            >
              {t('flow.dashboard.create_first', 'Criar primeiro funil')}
            </button>
          </div>
        ) : (
              ...
              >
                ...
                <div className="relative z-10">
                  ...
                  <p className="text-gray-500 text-sm mb-8 line-clamp-2 font-medium">
                    {funnel.description || t('flow.dashboard.default_description', 'Automatize seus processos visuais agora.')}
                  </p>
                  ...
                  <div className="pt-6 border-t border-white/5 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2 text-xs font-black uppercase italic tracking-tighter text-cyan-400 group-hover:gap-3 transition-all duration-500">
                      {t('flow.dashboard.open_editor', 'Abrir editor')}
      ...
      <ConfirmModal
        ...
        title={t('flow.dashboard.delete_title', 'Excluir projeto')}
        message={t('flow.dashboard.delete_message', 'Tem certeza que deseja excluir este projeto permanentemente? Esta ação não pode ser desfeita e todos os dados associados serão perdidos.')}
        confirmText={t('flow.dashboard.delete_confirm', 'Excluir')}
      ...
      <Modal
        ...
        title={t('flow.dashboard.edit_title', 'Editar projeto')}
      >
        <div className="space-y-6">
          <div className="space-y-2">
            <label className="text-sm font-bold text-white uppercase tracking-wider">
              {t('flow.dashboard.project_name', 'Nome do projeto')}
            </label>
            <input
              type="text"
              value={editName}
              ...
              placeholder={t('flow.dashboard.name_placeholder', 'Ex: Funil de lançamento')}
            />
          </div>

          <div className="space-y-2">
            <label className="text-sm font-bold text-white uppercase tracking-wider">
              {t('flow.dashboard.description', 'Descrição')}
            </label>
            <textarea
              ...
              placeholder={t('flow.dashboard.description_placeholder', 'Descreva o objetivo deste projeto...')}
            />
          </div>
          ...
          <div className="flex justify-end gap-3 pt-4 border-t border-white/10">
            ...
            >
              {t('flow.dashboard.save_changes', 'Salvar alterações')}
            </Button>
          </div>
        </div>
      </Modal>
