...
const EditorContent = () => {
  ...
  const onConfirmSave = async () => {
    ...
    if (success) {
      ...
      setIsSaving(false);
      console.error(t('flow.editor.save_error_log', 'Erro ao salvar funil.'));
    }
  };
    ...
    <div className="w-full h-screen bg-[#0a0a0b] overflow-hidden relative">
      {isSaving && (
        <div className="absolute top-8 right-8 z-50 flex items-center gap-3 px-5 py-2.5 glass rounded-2xl text-xs font-black uppercase tracking-widest text-[#27CBEF] shadow-2xl border-[#27CBEF]/20">
          ...
          {t('flow.editor.auto_save', 'Auto-Save')}
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-[#0a0a0b] border border-white/10 p-6 rounded-2xl shadow-2xl w-full max-w-md">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-bold text-white">{t('flow.editor.save_project', 'Salvar projeto')}</h2>
              <button onClick={() => setIsSaveModalOpen(false)} className="text-gray-400 hover:text-white transition-colors">
                <X size={20} />
              </button>
            </div>
            <div className="mb-6">
              <label className="block text-sm font-medium text-gray-400 mb-2">{t('flow.editor.funnel_name', 'Nome do funil')}</label>
              <input
                type="text"
                value={tempName}
                ...
                placeholder={t('flow.editor.name_placeholder', 'Ex: Lançamento semente')}
                autoFocus
              />
            </div>
