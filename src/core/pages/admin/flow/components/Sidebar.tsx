...
export const Sidebar = () => {
  ...
  const tabs = [
    { id: 'templates', icon: LayoutTemplate, label: t('flow.sidebar.tabs.templates') },
    { id: 'elements', icon: Plus, label: t('flow.sidebar.tabs.elements') },
    { id: 'layers', icon: Layers, label: t('flow.sidebar.tabs.layers') },
    { id: 'settings', icon: Settings, label: t('flow.sidebar.tabs.settings') },
  ];
  ...
  const localizeTemplateNodes = (template: any) => template.nodes.map((node: any) => ({
    ...node,
    data: {
      ...node.data,
  ...
  return (
    <>
      <div className="fixed left-0 top-0 bottom-0 z-[60] flex pointer-events-none">
        ...
        <div className="w-16 bg-[#0a0a0b] border-r border-white/5 flex flex-col items-center py-8 gap-6 pointer-events-auto shadow-2xl z-20">
          ...
          <button
            ...
            title={t('flow.sidebar.my_projects')}
          >
            <FolderOpen size={20} />
          </button>
          ...
          <div className="p-6 border-b border-white/5 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <h2 className="text-sm font-black uppercase tracking-[0.2em] text-white">
                {tabs.find(t => t.id === activeSidebarTab)?.label || t('flow.sidebar.menu')}
              </h2>
            </div>
            <button 
                ...
                <div className="flex items-center justify-between px-2">
                  <div className="flex items-center gap-2">
                    ...
                    <p className="text-[10px] font-black text-[#27CBEF] uppercase tracking-[0.2em] italic">
                      {t('flow.sidebar.ready_strategies')}
                    </p>
                  </div>
                  <div className="px-2 py-1 bg-[#27CBEF]/10 rounded-md border border-[#27CBEF]/20">
                    <span className="text-[8px] font-black text-[#27CBEF] uppercase">{t('flow.sidebar.items_count', { count: FUNNEL_TEMPLATES.length })}</span>
                  </div>
                </div>
                    ...
                    >
                      ...
                      <div className="relative z-10">
                        <div className="flex items-center justify-between mb-3">
                          <div className="flex items-center gap-3">
                            ...
                            <h3 className="text-[11px] font-black text-white group-hover:text-cyan-400 transition-colors uppercase tracking-tight italic">
                              {t(`flow.templates.${template.id}.name`, { defaultValue: template.name })}
                            </h3>
                          </div>
                        ...
                        <p className="text-[10px] text-gray-500 leading-relaxed line-clamp-2 font-medium mb-4 group-hover:text-gray-400 transition-colors">
                          {t(`flow.templates.${template.id}.description`, { defaultValue: template.description })}
                        </p>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[8px] font-black text-gray-600 uppercase tracking-widest group-hover:text-gray-500 transition-colors">
                            {t('flow.sidebar.template_stats', { blocks: template.nodes.length, connections: template.edges.length })}
                          </span>
                        </div>
                      </div>
            ...
            {activeSidebarTab !== 'templates' && (
              <div className="flex flex-col items-center justify-center h-full text-center p-8 animate-in fade-in zoom-in-95 duration-700">
                ...
                <p className="text-[10px] font-black text-white uppercase tracking-[0.3em] italic">
                  {t('flow.sidebar.premium_feature')}
                </p>
                <p className="text-[9px] text-slate-600 mt-3 max-w-[200px] font-medium leading-relaxed uppercase tracking-widest">
                  {t('flow.sidebar.coming_soon_desc', {
                    area: activeSidebarTab === 'elements'
                      ? t('flow.sidebar.areas.visual_elements')
                      : activeSidebarTab === 'layers'
                        ? t('flow.sidebar.areas.layer_management')
                        : t('flow.sidebar.areas.global_settings')
                  })}
                </p>
                
                <div className="mt-8 px-4 py-2 bg-cyan-500/5 rounded-xl border border-cyan-500/10">
                  <span className="text-[8px] font-black text-cyan-500/60 uppercase tracking-tighter">{t('flow.sidebar.in_development')}</span>
                </div>
              </div>
            )}
          </div>
      ...
      <ConfirmationModal
        ...
        onConfirm={() => {
          if (pendingTemplate) {
            loadTemplate(localizeTemplateNodes(pendingTemplate), pendingTemplate.edges);
            setIsSidebarOpen(false);
          }
        }}
        title={t('flow.sidebar.replace_title')}
        message={t('flow.sidebar.replace_message', { name: pendingTemplate ? t(`flow.templates.${pendingTemplate.id}.name`, { defaultValue: pendingTemplate.name }) : '' })}
      />
    </>
  );
};
