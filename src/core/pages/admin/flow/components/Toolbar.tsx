...
export const Toolbar = ({ onAutoLayout, onExportImage, onExportJson, onSave }: { 
  ...
  const handleAddNode = (item: any, category: string) => {
    ...
    const categoryToSet = item.type === 'product' ? 'Produto' : (item.type === 'note' ? 'Nota' : category);
  ...
  const activeGroup = activeCategory === 'quick-add' 
    ? { category: 'Adicionar Rápido', icon: Plus, color: 'text-white', items: QUICK_ADD_ITEMS }
    : NODE_TYPES.find(g => g.id === activeCategory);
        ...
        <div className="flex items-center gap-1 px-2 border-r border-white/5">
          ...
          {NODE_TYPES.map((group) => (
            ...
            >
              ...
              <span className="text-[10px] font-black uppercase tracking-widest hidden md:block">{group.category}</span>
            </button>
          ))}
        </div>
