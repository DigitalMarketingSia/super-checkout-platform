...
export const ConfigModal = () => {
  ...
  const tabs = [
    { id: 'general', label: t('flow.config.tabs.general'), icon: Layout, color: 'text-blue-500', bg: 'bg-blue-500/10' },
    { id: 'planning', label: t('flow.config.tabs.planning'), icon: Target, color: 'text-emerald-500', bg: 'bg-emerald-500/10' },
    { id: 'strategy', label: t('flow.config.tabs.strategy'), icon: FileText, color: 'text-amber-500', bg: 'bg-amber-500/10' },
  ];

  const statusOptions: { value: NodeStatus; label: string; color: string }[] = [
    { value: 'ativo', label: t('flow.status.ativo'), color: 'bg-emerald-500' },
    { value: 'em_teste', label: t('flow.status.em_teste'), color: 'bg-amber-500' },
    { value: 'pausado', label: t('flow.status.pausado'), color: 'bg-slate-500' },
  ];
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                      <div className="space-y-6">
                        <div className="space-y-2">
                          <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">{t('flow.config.element_name')}</label>
                          <input
                            type="text"
                            ...
                            placeholder={t('flow.config.node_name_placeholder')}
                          />
                        </div>
                        ...
                        {node.data.isNote ? (
                          <div className="space-y-6">
                            <div className="space-y-2">
                              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">{t('flow.config.note_content')}</label>
                              <textarea
                                ...
                                placeholder={t('flow.config.note_placeholder')}
                              />
                            </div>
                          ...
                          <div className="space-y-6">
                            <div className="space-y-2">
                              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">{t('flow.config.product_type')}</label>
                              <div className="grid grid-cols-2 gap-2">
                                {[
                                  { id: 'principal', label: t('flow.product_types.main'), color: 'bg-purple-500', icon: Package },
                                  { id: 'gratis', label: t('flow.product_types.free'), color: 'bg-emerald-500', icon: CheckCircle },
                            ...
                            <div className="space-y-2">
                              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">{t('flow.config.product_value')}</label>
                            ...
                            <div className="space-y-2">
                              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">{t('flow.config.repurchase_ltv')}</label>
                              <div className="relative group">
                          ...
                          <div className="space-y-6">
                            <div className="space-y-2">
                              <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">{t('flow.config.initial_visits')}</label>
                              <div className="relative group">
                                ...
                                <input
                                  ...
                                  placeholder={t('flow.config.initial_visits_placeholder')}
                                  className="w-full bg-white/[0.03] border border-white/10 rounded-2xl pl-16 pr-6 py-4 text-lg text-white focus:border-emerald-500/50 focus:ring-4 focus:ring-emerald-500/5 transition-all outline-none"
                                />
                              </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-6">
                              <div className="space-y-2">
                                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">{t('flow.config.cpc')}</label>
                                <div className="relative group">
                          ...
                          <div className="space-y-2">
                            <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">{t('flow.config.destination_url')}</label>
                            <div className="relative group">
                      ...
                      <div className="space-y-2">
                        <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">{t('flow.config.operational_status')}</label>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                      <div className="space-y-8">
                        <div className="p-8 glass rounded-[32px] border-white/10 bg-white/5 space-y-4">
                          <div className="flex items-center gap-3">
                            ...
                            <h4 className="text-xs font-black text-white uppercase tracking-widest">{t('flow.config.flow_metrics')}</h4>
                          </div>
                          <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-1">
                              <p className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">{t('flow.config.estimated_volume')}</p>
                              <p className="text-2xl font-black text-white">{Math.round(node.data.volume || 0).toLocaleString()}</p>
                            </div>
                            <div className="space-y-1">
                              <p className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">{t('flow.config.unit_value')}</p>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                          <div className="p-6 glass rounded-[24px] border-white/10 bg-white/5 space-y-2">
                            <p className="text-[9px] font-bold text-slate-500 uppercase tracking-widest">{t('flow.config.investment_cost')}</p>
                        ...
                        <div className="p-8 glass rounded-[32px] border-white/10 bg-white/5 space-y-4">
                          <div className="flex items-center gap-3">
                            ...
                            <h4 className="text-xs font-black text-white uppercase tracking-widest">{t('flow.config.break_even')}</h4>
                          </div>
                          <div className="space-y-2">
                            <p className="text-[10px] text-slate-400 leading-relaxed">
                              {t('flow.config.break_even_prefix')} <strong>R$ {((node.data.cost as number) || 0).toLocaleString()}</strong> {t('flow.config.break_even_middle')}
                              <strong>{Math.ceil(((node.data.cost as number) || 0) / (node.data.price || 1))}</strong> {t('flow.config.break_even_suffix')}
                            </p>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="flex flex-col justify-center">
                        <div className="p-12 glass rounded-[40px] border-[#27CBEF]/20 bg-[#27CBEF]/5 text-center space-y-4">
                          <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.4em]">{t('flow.config.total_estimated_revenue')}</p>
                          ...
                          <p className="text-[10px] text-slate-500 font-medium">
                            {t('flow.config.including_repurchase', { percent: node.data.recompra || 0 })}
                          </p>
                        </div>
                      </div>
                    </div>
                ...
                {activeTab === 'strategy' && (
                  ...
                  >
                    <textarea
                      value={node.data.notes || ''}
                      onChange={(e) => updateNodeData(node.id, { notes: e.target.value })}
                      placeholder={t('flow.config.strategy_placeholder')}
