    ...
    <Layout>
      ...
      >
        ...
        <div className="flex flex-col gap-6 relative z-10">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-2 text-white/60 text-[10px] font-mono uppercase tracking-[0.2em]">
                ...
                {activeTab === 'orders'
                  ...
                  : t('orders.header.user_profiles', { count: customers.length })}
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-row flex-nowrap overflow-x-auto gap-3 mt-4 pb-2 md:pb-0 scrollbar-hide">
            <div className="bg-black/40 px-4 py-3 rounded-2xl border border-white/5 flex items-center gap-4 group hover:border-emerald-500/30 transition-all shrink-0">
              ...
              <div>
                <p className="text-[8px] font-black uppercase tracking-widest text-emerald-500/60 mb-0.5 whitespace-nowrap">{t('orders.metrics.total_revenue')}</p>
            ...
            <div className="bg-black/40 px-4 py-3 rounded-2xl border border-white/5 flex items-center gap-4 group hover:border-blue-500/30 transition-all shrink-0">
              ...
              <div>
                <p className="text-[8px] font-black uppercase tracking-widest text-blue-500/60 mb-0.5 whitespace-nowrap">{t('orders.metrics.total_sales')}</p>
            ...
            <div className="bg-black/40 px-4 py-3 rounded-2xl border border-white/5 flex items-center gap-4 group hover:border-primary/30 transition-all shrink-0">
              ...
              <div>
                <p className="text-[8px] font-black uppercase tracking-widest text-primary/60 mb-0.5 whitespace-nowrap">{t('orders.metrics.unique_leads')}</p>
        ...
        <div className="flex flex-col items-end gap-6 relative z-10 shrink-0">
          ...
          <div className="hidden lg:flex flex-col items-end">
            <span className="text-[10px] font-black text-white/20 uppercase tracking-[0.4em] mb-2 leading-none">{t('orders.header.security_protocol')}</span>
            <div className="flex items-center gap-2">
              <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
              <span className="text-[10px] font-mono text-emerald-500/60 uppercase tracking-widest">{t('orders.header.active_scan')}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mb-0 flex flex-col lg:flex-row gap-4 items-center justify-between bg-black/40 p-4 rounded-t-[2.5rem] border-x border-t border-white/5 backdrop-blur-xl">
         <div className="w-full lg:w-96 relative group">
            ...
            <input 
              type="text" 
              placeholder={activeTab === 'orders' ? t('orders.filters.scan_orders') : t('orders.filters.scan_customers')}
         ...
         <div className="flex items-center gap-3 w-full lg:w-auto">
            <div className="relative flex-1 lg:w-64 group/s">
               ...
               >
                 <option value="all" className="bg-[#0F0F15]">{t('orders.filters.all_nodes')}</option>
                 {products.map(p => (
      ...
      {showFilters && (
        <div className="p-8 bg-black/60 border-x border-white/5 grid grid-cols-1 md:grid-cols-2 gap-8 animate-in slide-in-from-top-2 relative overflow-hidden group/drawer">
          ...
          <div className="relative z-10">
            <label className="text-[10px] text-white/20 font-black uppercase tracking-[0.3em] mb-4 block ml-1 italic">{t('orders.filters.registry_status')}</label>
          ...
          <div className="relative z-10">
            ...
            <div className="flex flex-wrap gap-2">
               {['all', 'credit_card', 'pix', 'boleto'].map(m => (
                 ...
                 >
                   {m === 'all' ? t('orders.filters.all_protocols') : m.toUpperCase()}
                 </button>
               ))}
            </div>
          </div>
          ...
          <div className="flex flex-col">
            {filteredOrders.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-40 text-white/5">
                ...
                <p className="text-[10px] font-black uppercase tracking-[0.4em] italic">{t('orders.empty.no_transactions')}</p>
              </div>
            ) : (
        ...
        <div className="mt-12 flex flex-col items-center gap-6">
          ...
          <p className="text-[10px] text-gray-700 font-black uppercase tracking-[0.2em]">
            {t('orders.pagination.summary', {
              page: currentPage,
              totalPages,
              total: activeTab === 'orders' ? filteredOrders.length : filteredCustomers.length
            })}
          </p>
        </div>
      )}
