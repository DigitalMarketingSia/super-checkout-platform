...
import { Plus, Trash2, Tag, Zap, Layers } from 'lucide-react';
...
export const Offers = () => {
  ...
  const locale = i18n.language.startsWith('pt')
    ? 'pt-BR'
  ...
  return (
    <Layout>
      <div className="flex flex-col lg:flex-row justify-between lg:items-end mb-12 gap-8">
        <div>
          <h1 className="text-4xl lg:text-5xl font-portal-display text-white mb-2 uppercase leading-none">{t('offers.title')}</h1>
          <div className="flex items-center gap-3">
             <p className="text-gray-600 font-medium uppercase tracking-[0.1em] text-[10px]">{t('offers.subtitle')}</p>
             <div className="h-1 w-1 rounded-full bg-gray-800"></div>
             <span className="text-[10px] text-primary font-black uppercase tracking-[0.2em]">{t('offers.badge')}</span>
          </div>
        </div>
        <Button onClick={() => setIsModalOpen(true)} className="px-10 py-4 bg-primary text-white rounded-[1.5rem] shadow-2xl shadow-primary/30 border-none font-black uppercase tracking-widest text-xs flex items-center gap-3 active:scale-95 transition-all">
          <Plus className="w-5 h-5" /> {t('offers.create_btn')}
        </Button>
      </div>
      ...
      {offers.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-32 text-center">
           ...
           <h3 className="text-2xl font-portal-display text-white uppercase tracking-tight opacity-40">{t('offers.no_offers')}</h3>
        </div>
      ) : (
                ...
                <div className="relative z-20 flex flex-col lg:flex-row lg:items-center gap-6 lg:gap-10">
                   ...
                   <div className="flex items-center gap-4 lg:w-72 shrink-0">
                      ...
                      <div className="min-w-0">
                        <p className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-700 mb-1">{t('offers.labels.offer_name')}</p>
                   ...
                   <div className="flex-1 min-w-0">
                      <p className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-700 mb-1">{t('offers.labels.linked_product')}</p>
                      <div className="flex items-center gap-2">
                         <Layers className="w-3.5 h-3.5 text-gray-600" />
                         <span className="text-xs font-bold text-gray-400 truncate">{product?.name || t('offers.unknown_product')}</span>
                      </div>
                   </div>

                   {/* Pricing */}
                   <div className="lg:w-48 shrink-0">
                      <p className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-700 mb-1">{t('offers.labels.plan_value')}</p>
                      <div className="flex items-baseline gap-1">
                         <span className="text-2xl font-portal-display text-white">{formatCurrency(offer.price)}</span>
                         {isRecurring && (
                           <span className="text-[10px] font-black text-gray-700 uppercase tracking-widest">
                             / {offer.recurrence_type === RecurrenceType.MONTHLY ? t('offers.period.month') : t('offers.period.year')}
                           </span>
                         )}
                      </div>
                   </div>
                   ...
                   <div className="lg:w-64 flex items-center justify-between lg:justify-end gap-6 shrink-0">
                      <div className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-[0.2em] border ${isRecurring ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' : 'bg-white/5 text-gray-500 border-white/5'}`}>
                         {isRecurring ? t('offers.type.recurring') : t('offers.type.one_time')}
                      </div>
      ...
      {/* Create Modal */}
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title={t('offers.form.modal_title')}>
        ...
        <form onSubmit={handleSubmit} className="relative z-10 space-y-6 p-2">
          <div>
            <label className="block text-[10px] font-black text-gray-600 uppercase tracking-widest mb-3">{t('offers.form.name_label')}</label>
            <input
              required type="text"
              ...
              placeholder={t('offers.form.name_placeholder')}
            />
          </div>

          <div>
            <label className="block text-[10px] font-black text-gray-600 uppercase tracking-widest mb-3">{t('offers.form.product_label')}</label>
            <select
              required
            ...
            >
              <option value="">{t('offers.form.product_placeholder')}</option>
              {products.map(p => (
          ...
          <div className="grid grid-cols-2 gap-6">
            <div>
              <label className="block text-[10px] font-black text-gray-600 uppercase tracking-widest mb-3">{t('offers.form.price_label')}</label>
              <div className="relative">
                 <span className="absolute left-5 top-1/2 -translate-y-1/2 text-gray-700 font-black text-xs">{currencySymbol}</span>
                 <input
            ...
            <div>
              <label className="block text-[10px] font-black text-gray-600 uppercase tracking-widest mb-3">{t('offers.form.cycle_type_label')}</label>
              <select
                className="w-full bg-black/40 border border-white/5 rounded-2xl px-5 py-4 focus:border-primary/50 outline-none text-white text-sm font-medium appearance-none cursor-pointer [&>option]:bg-[#0F0F15]"
                value={formData.payment_type}
                onChange={e => setFormData({ ...formData, payment_type: e.target.value as PaymentType })}
              >
                <option value={PaymentType.ONE_TIME}>{t('offers.type.one_time')}</option>
                <option value={PaymentType.RECURRING}>{t('offers.form.cycle_type_subscription')}</option>
              </select>
            </div>
          </div>
          ...
          {formData.payment_type === PaymentType.RECURRING && (
            <div className="animate-in fade-in slide-in-from-top-2">
              <label className="block text-[10px] font-black text-gray-600 uppercase tracking-widest mb-3">{t('offers.form.billing_frequency_label')}</label>
              <div className="grid grid-cols-2 gap-4">
                 {[RecurrenceType.MONTHLY, RecurrenceType.YEARLY].map(type => (
                   ...
                   >
                     {type === RecurrenceType.MONTHLY ? t('offers.form.billing_frequency_monthly') : t('offers.form.billing_frequency_yearly')}
                   </button>
                 ))}
              </div>
            </div>
          ...
          <div className="pt-8 flex justify-end gap-4">
            <button type="button" onClick={() => setIsModalOpen(false)} className="px-6 py-3 text-[10px] font-black text-gray-600 uppercase tracking-widest hover:text-white transition-colors">{t('common.cancel')}</button>
            <Button type="submit" className="px-8 py-4 bg-white text-black font-black uppercase text-xs tracking-widest rounded-2xl shadow-xl transition-all active:scale-95">{t('offers.form.submit')}</Button>
          </div>
        </form>
      </Modal>
      ...
      <ConfirmModal
        ...
        message={t('offers.delete.message')}
        confirmText={t('offers.delete.confirm')}
        cancelText={t('offers.delete.cancel')}
        variant="danger"
        loading={isDeleting}
      />
