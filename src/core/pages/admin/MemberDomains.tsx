    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center py-12">
                ...
                <p className="text-[10px] font-mono text-white/20 uppercase tracking-widest">{t('member_area_details.domains_loading', 'Resolving DNS nodes...')}</p>
            </div>
        );
    }
            ...
            <div className="bg-black/40 border border-white/5 rounded-[2rem] p-8 lg:p-10 relative overflow-hidden group hover:border-white/10 transition-all">
                ...
                <div className="flex flex-col lg:flex-row gap-10 relative z-10">
                    <div className="lg:w-1/3">
                        ...
                        <h3 className="text-xl font-black text-white italic uppercase tracking-tighter mb-2">
                            {t('member_area_details.domains_heading.prefix', 'Custom')} <span style={{ color: primaryColor }}>{t('member_area_details.domains_heading.highlight', 'Domain')}</span>
                        </h3>
                        <p className="text-white/30 text-xs font-medium leading-relaxed">
                            {t('member_area_details.domains_heading.subtitle', 'Connect a verified external mapping to provide a white-label experience for your students.')}
                        </p>
                    </div>

                    <div className="lg:w-2/3 space-y-8">
                        <div>
                            <label className="text-[10px] font-bold text-white/40 uppercase tracking-widest mb-3 block ml-1">{t('member_area_details.domains_mapping_label', 'Mapping node configuration')}</label>
                            <div className="relative group/select">
                                ...
                                >
                                    <option value="">{t('member_area_details.domains_default_gateway', 'System default gateway (super-checkout.net)')}</option>
                                    {domains.map(domain => (
                            ...
                            <div className="flex items-center gap-3 mt-4 px-2">
                                ...
                                <div className="flex items-center gap-2 text-[10px] font-mono text-white/40 uppercase tracking-widest">
                                    ...
                                    <span>{t('member_area_details.domains_save_hint', 'Commit changes via the global save button above')}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {selectedDomainId ? (
                                <div className="flex items-center gap-4 p-5 bg-green-500/5 border border-green-500/20 rounded-2xl group/status">
                                    ...
                                    <div>
                                        <p className="text-xs font-black text-white uppercase italic tracking-tighter">{t('member_area_details.domains_status_active', 'Status: active')}</p>
                                        <p className="text-[10px] font-medium text-green-500/60 uppercase tracking-widest">{t('member_area_details.domains_status_verified', 'Node verified')}</p>
                                    </div>
                                </div>
                            ) : (
                                <div className="flex items-center gap-4 p-5 bg-white/5 border border-white/10 rounded-2xl group/status opacity-40">
                                    ...
                                    <div>
                                        <p className="text-xs font-black text-white uppercase italic tracking-tighter">{t('member_area_details.domains_status_passive', 'Status: passive')}</p>
                                        <p className="text-[10px] font-medium text-white/40 uppercase tracking-widest">{t('member_area_details.domains_status_core', 'Using core domain')}</p>
                                    </div>
                                </div>
                            )}
                            ...
                            {domains.length === 0 && (
                                <div className="flex items-center gap-4 p-5 bg-yellow-500/5 border border-yellow-500/20 rounded-2xl group/status">
                                    ...
                                    <div>
                                        <p className="text-xs font-black text-white uppercase italic tracking-tighter">{t('member_area_details.domains_no_nodes', 'No nodes found')}</p>
                                        <p className="text-[10px] font-medium text-yellow-500/60 uppercase tracking-widest leading-tight">{t('member_area_details.domains_configure_tab', 'Configure in domains tab')}</p>
                                    </div>
                                </div>
                            )}
                        </div>
                ...
                <div className="mt-10 pt-6 border-t border-white/5 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-[8px] font-mono text-white/10 uppercase tracking-[0.2em]">{t('member_area_details.domains_engine', 'Network mapping engine v4.0')}</span>
                    </div>
