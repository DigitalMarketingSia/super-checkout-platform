...
export const Webhooks = () => {
   ...
   return (
      <Layout
         title={t('webhooks.layout_title', 'Webhooks e integrações')}
         subtitle={t('webhooks.layout_subtitle', 'Conecte seu checkout a ferramentas externas com baixa latência.')}
      >
         {/* Top Header Decor */}
         ...
         <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
            <div>
               <h1 className="text-5xl font-portal-display text-white italic tracking-tighter mb-4">{t('webhooks.title', 'Central de webhooks')}</h1>
               <p className="text-gray-500 font-medium max-w-xl">{t('webhooks.subtitle', 'Orquestre o fluxo de dados em tempo real. Configure endpoints de saída ou utilize nossa API de borda para atualizações remotas.')}</p>
            </div>
            <Button 
               onClick={openNew}
            ...
            >
               <Plus className="w-5 h-5" /> {t('webhooks.new_integration', 'Nova integração')}
            </Button>
         </div>
         ...
         <div className="relative z-10 flex p-1.5 bg-white/5 backdrop-blur-xl rounded-[1.5rem] border border-white/5 mb-10 w-fit">
            ...
            >
               {t('webhooks.tabs.outgoing', 'Saída')}
            </button>
            <button
            ...
            >
               {t('webhooks.tabs.history', 'Histórico')}
            </button>
         </div>
               ...
               <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  {webhooks.length === 0 ? (
                     <div className="text-center py-24 bg-white/5 rounded-[2.5rem] border border-dashed border-white/10">
                        ...
                        <h3 className="text-xl font-bold text-white mb-2">{t('webhooks.empty_title', 'Sem conexões ativas')}</h3>
                        <p className="text-gray-500 mb-10 max-w-sm mx-auto">{t('webhooks.empty_desc', 'Sincronize seu fluxo de dados com ferramentas externas em segundos.')}</p>
                        <Button 
                           onClick={openNew}
                           className="rounded-2xl bg-white/5 hover:bg-white/10 text-white font-bold border border-white/10 px-8 py-4"
                        >
                           {t('webhooks.start_integration', 'Iniciar integração')}
                        </Button>
                     </div>
                  ) : (
                              ...
                              <div className="p-8 flex flex-col lg:flex-row items-center gap-8">
                                 <div className="flex flex-col items-center">
                                    ...
                                    <span className="text-[9px] font-black tracking-widest text-gray-700 mt-2 uppercase">
                                       {wh.active ? t('webhooks.status.active', 'Ativo') : t('webhooks.status.paused', 'Pausado')}
                                    </span>
                                 </div>
                                 ...
                                 <div className="flex items-center gap-3">
                                    <Button size="sm" onClick={() => handleTest(wh)} variant="ghost" className="bg-white/5 hover:bg-white/10 rounded-xl h-12 px-6 font-bold border border-white/5">
                                       <Play className="w-4 h-4 text-primary mr-2" /> {t('common.test', 'Testar')}
                                    </Button>
            ...
            {activeTab === 'incoming' && (
               <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 max-w-5xl space-y-10">
                  <div className="bg-gradient-to-br from-primary/10 via-purple-500/5 to-transparent rounded-[2.5rem] p-10 border border-white/5 mb-10 overflow-hidden relative">
                     ...
                     <div className="relative z-10">
                        <h2 className="text-3xl font-portal-display text-white italic tracking-tight mb-4">{t('webhooks.incoming_title', 'Webhooks de entrada')}</h2>
         ...
         <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
            <div className="p-8">
               <h2 className="text-2xl font-portal-display text-white italic mb-6">
                  {editingWebhook ? t('webhooks.configure_webhook', 'Configurar webhook') : t('webhooks.new_integration', 'Nova integração')}
               </h2>
               <form onSubmit={handleSubmit} className="space-y-6">
                  <div>
                     <label className="block text-[10px] font-black uppercase text-gray-500 mb-2">{t('webhooks.form.name', 'Identificação')}</label>
                     <input 
                        type="text" 
                        value={formData.name}
                        ...
                        placeholder={t('webhooks.form.name_placeholder', 'Ex: Integração CRM Alpha')}
                     />
                  </div>
                  <div>
                  ...
                  <div className="flex justify-end gap-3 mt-8">
                     <Button variant="ghost" onClick={() => setIsModalOpen(false)}>{t('common.cancel', 'Cancelar')}</Button>
                     <Button type="submit">{t('webhooks.form.save', 'Salvar webhook')}</Button>
                  </div>
               </form>
            </div>
         </Modal>
