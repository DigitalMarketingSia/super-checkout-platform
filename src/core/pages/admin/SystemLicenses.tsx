...
const isProtectedSystemLicense = (license: License) => {
    ...
    return PROTECTED_SYSTEM_DOMAINS.includes(domain)
        ...
        <Layout>
            <div className="mb-12 flex flex-col md:flex-row md:items-center justify-between gap-6 animate-in fade-in slide-in-from-top duration-700">
                <div>
                    <h1 className="text-4xl font-black text-white italic tracking-tighter flex items-center gap-3">
                        ...
                        {t('system_licenses.title_main', 'Licenças')} <span className="text-primary">{t('system_licenses.title_accent', 'Sistema')}</span>
                    </h1>
                    <div className="flex items-center gap-3 mt-2">
                        <span className="flex items-center gap-1.5 bg-primary/10 text-primary px-3 py-1 rounded-full text-[10px] font-bold tracking-widest uppercase border border-primary/20">
                           <Shield className="w-3 h-3" /> {t('system_licenses.protocol_active', 'Protocolo de segurança ativo')}
                        </span>
                        <span className="text-gray-500 text-xs font-medium">{t('system_licenses.subtitle', 'Controle central de planos e licenças')}</span>
                    </div>
                </div>
                ...
                <div className="relative group">
                    ...
                    <Card className="relative p-1.5 bg-[#0A0A15]/60 backdrop-blur-2xl border-white/5 rounded-2xl overflow-hidden">
                        <div className="relative flex items-center">
                            ...
                            <input
                                type="text"
                                placeholder={t('system_licenses.search_placeholder', 'Buscar licenças por nome, e-mail ou chave...')}
                                value={search}
                ...
                <Card className="overflow-hidden bg-[#0A0A15]/40 backdrop-blur-2xl border-white/5 rounded-[2.5rem]">
                    <div className="overflow-x-auto">
                        <table className="w-full text-left text-sm text-gray-400">
                            <thead className="bg-white/[0.02] text-gray-300 font-bold uppercase tracking-widest text-[10px]">
                                <tr>
                                    <th className="pl-8 pr-4 py-6">{t('system_licenses.table.identity', 'Identidade')}</th>
                                    <th className="px-4 py-6">{t('system_licenses.table.plan', 'Plano')}</th>
                                    <th className="px-4 py-6">{t('system_licenses.table.endpoint', 'Instalação')}</th>
                                    <th className="px-4 py-6 text-center">{t('common.status', 'Status')}</th>
                                    <th className="pl-4 pr-8 py-6 text-right">{t('common.actions', 'Ações')}</th>
                                </tr>
                            </thead>
                                    ...
                                    <tr>
                                        <td colSpan={5} className="p-20 text-center">
                                            <div className="flex flex-col items-center gap-4">
                                                ...
                                                <span className="text-gray-500 font-medium animate-pulse">{t('system_licenses.loading', 'Carregando licenças...')}</span>
                                            </div>
                                        </td>
                                    </tr>
                                ) : fetchError ? (
                                    ...
                                    <tr>
                                        <td colSpan={5} className="p-20 text-center">
                                            <div className="flex flex-col items-center gap-4 opacity-20">
                                                ...
                                                <span className="text-xl font-bold tracking-tighter italic">{t('system_licenses.empty', 'Nenhuma licença encontrada')}</span>
                                            </div>
                                        </td>
                                    </tr>
                                ) : (
                                                ...
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <div>
                                                        <div className="text-white font-black italic tracking-tight text-lg leading-tight uppercase underline decoration-primary/30 decoration-2 underline-offset-4 group-hover:decoration-primary transition-all">
                                                            {lic.client_name || t('system_licenses.anonymous', 'Sem nome')}
                                                        </div>
                                                        ...
                                                        {isProtected && (
                                                            <div className="mt-2 w-fit inline-flex items-center gap-1.5 rounded-full border border-yellow-400/25 bg-yellow-400/10 px-2.5 py-1 text-[9px] font-black uppercase tracking-widest text-yellow-300">
                                                                ...
                                                                {t('system_licenses.system_owner', 'Owner do sistema')}
                                                            </div>
                                                        )}
                                            ...
                                            <td className="px-4 py-6">
                                                <div className="flex flex-col gap-1.5">
                                                    ...
                                                    <div className="flex items-center gap-1.5 text-[10px] text-gray-500 font-bold uppercase tracking-tight pl-1">
                                                        ...
                                                        {lic.max_instances > 900000 ? t('system_licenses.unlimited_nodes', 'Instalações ilimitadas') : t('system_licenses.max_nodes', '{{count}} instalações', { count: lic.max_instances })}
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="px-4 py-6">
                                                {lic.current_domain ? (
                                                    <div className="flex flex-col gap-1">
                                                        ...
                                                        <span className="text-[10px] text-gray-600 font-medium">{t('system_licenses.active_endpoint', 'Instalação ativa')}</span>
                                                    </div>
                                                ) : (
                                                    <span className="text-gray-700 italic text-xs">{t('system_licenses.waiting_deployment', 'Aguardando instalação...')}</span>
                                                )}
                                            </td>
                                            ...
                                            <td className="pl-4 pr-8 py-6 text-right">
                                                <div className="flex items-center justify-end gap-2">
                                                    <Button 
                                                        ...
                                                        title={t('system_licenses.manage_resources', 'Gerenciar recursos')}
                                                    >
                                                    ...
                                                    <Button 
                                                        ...
                                                        title={t('system_licenses.monitoring_nodes', 'Monitorar instalações')}
                                                    >
                    ...
                    <div className="p-6 border-t border-white/5 flex flex-col sm:flex-row items-center justify-between gap-4 bg-white/[0.01]">
                        <div className="text-[10px] font-bold uppercase tracking-widest text-gray-500">
                            {t('system_licenses.pagination.showing_prefix', 'Mostrando')} <span className="text-white">{(page - 1) * 10 + 1} — {Math.min(page * 10, totalItems)}</span> {t('system_licenses.pagination.of', 'de')} <span className="text-primary">{totalItems}</span> {t('system_licenses.pagination.items', 'licenças')}
                        </div>
                                        ...
                                        <div className="relative flex justify-between items-center">
                                            <div>
                                                ...
                                                <div className="text-[10px] font-bold uppercase tracking-widest text-gray-500 mt-0.5">
                                                    {plan.maxInstallations > 900000 ? t('system_licenses.unlimited_node_infra', 'Instalações ilimitadas') : t('system_licenses.max_nodes_label', 'Máximo: {{count}} instalações', { count: plan.maxInstallations })}
                                                </div>
                                            </div>
            ...
            >
                <div className="space-y-6">
                    <div className="flex items-center justify-between p-4 bg-white/5 border border-white/5 rounded-2xl animate-in fade-in duration-500">
                        <div>
                            <div className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-1">{t('system_licenses.target_protocol', 'Licença selecionada')}</div>
                    ...
                    {loadingInstallations ? (
                        <div className="flex flex-col items-center justify-center p-12 gap-4">
                            ...
                            <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest">{t('system_licenses.scanning_network', 'Carregando instalações...')}</span>
                        </div>
                        ...
                        <div className="text-center p-12 border-2 border-dashed border-white/5 rounded-[2rem] bg-white/[0.02]">
                            ...
                            <div className="text-xl font-bold text-gray-600 italic tracking-tighter uppercase">{t('system_licenses.no_nodes', 'Nenhuma instalação ativa')}</div>
                            <p className="text-xs text-gray-500 mt-2 font-medium">{t('system_licenses.no_nodes_desc', 'Esta licença ainda não foi instalada em nenhum domínio.')}</p>
                        </div>
                    ) : (
                            ...
                            {selectedInstallations.map((inst) => (
                                <div key={inst.id} className="group bg-[#0A0A15]/40 border border-white/5 p-4 rounded-2xl flex items-center justify-between hover:bg-white/[0.05] hover:border-white/10 transition-all duration-300">
                                    ...
                                    <a
                                        ...
                                        title={t('system_licenses.access_point', 'Acessar instalação')}
                                    >
            ...
            >
                <div className="space-y-6">
                    ...
                    {loadingFeatures ? (
                        <div className="flex flex-col items-center justify-center p-12 gap-4">
                            <RefreshCw className="w-8 h-8 animate-spin text-primary" />
                            <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest">{t('system_licenses.loading_features', 'Carregando recursos...')}</span>
                        </div>
                    ) : (
            ...
            >
                <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                        <div className="group">
                            <label className="block text-[10px] font-black text-gray-500 mb-2 uppercase tracking-widest pl-1">{t('system_licenses.deployment_access', 'Link de instalação para o cliente')}</label>
                            <div className="relative">
                                <input
                                    type="text"
                                    readOnly
                        ...
                        <div className="group">
                            <label className="block text-[10px] font-black text-gray-500 mb-2 uppercase tracking-widest pl-1">{t('system_licenses.license_key_label', 'Chave da licença')}</label>
                            <div className="relative">
                    ...
                    <div className="pt-4 flex justify-end gap-3">
                        ...
                        >
                            {t('system_licenses.open_installer', 'Abrir instalador')}
                        </Button>
                        <Button 
            ...
            <ConfirmModal
                ...
                title={
                    actionModal.type === 'delete' ? t('system_licenses.confirm.delete_title', 'Exclusão permanente') :
                    actionModal.type === 'suspend' ? t('system_licenses.confirm.suspend_title', 'Suspender licença') : t('system_licenses.confirm.restore_title', 'Restaurar licença')
                }
                message={
