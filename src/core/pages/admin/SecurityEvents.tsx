...
const eventLabels: Record<string, string> = {
  ...
  two_factor_secret_missing: 'Configuracao 2FA incompleta',
  two_factor_session_missing: 'Sessao 2FA ausente',
  two_factor_challenge_create_failed: 'Falha ao criar challenge 2FA',
  two_factor_inspection_failed: 'Falha ao inspecionar 2FA',
  two_factor_challenge_lookup_failed: 'Falha ao buscar challenge 2FA',
  two_factor_challenge_replay: 'Reuso de challenge 2FA',
  two_factor_challenge_expired: 'Challenge 2FA expirado',
  two_factor_challenge_decryption_failed: 'Falha ao decifrar challenge 2FA',
  two_factor_session_invalid: 'Sessao 2FA invalida',
    ...
    <Layout>
      ...
      <div className="mb-12 flex flex-col md:flex-row md:items-center justify-between gap-6 animate-in fade-in slide-in-from-top duration-700">
        <div>
          <h1 className="text-4xl font-black text-white italic tracking-tighter flex items-center gap-3">
            ...
            {t('security_events.title_main', 'Auditoria')} <span className="text-primary">{t('security_events.title_accent', 'Segurança')}</span>
          </h1>
          <div className="flex items-center gap-3 mt-2">
            <span className="flex items-center gap-1.5 bg-primary/10 text-primary px-3 py-1 rounded-full text-[10px] font-bold tracking-widest uppercase border border-primary/20">
               <Terminal className="w-3 h-3" /> {t('security_events.integrity_active', 'Monitoramento ativo')}
            </span>
            <span className="text-gray-500 text-xs font-medium">{t('security_events.subtitle', 'Logs de autorização em tempo real')}</span>
          </div>
        </div>
        <div className="flex gap-3">
            ...
            <div className="px-6 h-12 flex items-center bg-[#0A0A15]/60 border border-white/5 rounded-xl">
                <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest">{t('security_events.protocol_live', 'Protocolo ativo')}</span>
                </div>
            </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12 animate-in fade-in slide-in-from-bottom duration-700 delay-100">
        ...
        >
          ...
          <div className="text-[10px] uppercase font-black text-gray-500 tracking-[0.2em] mb-4">{t('security_events.metrics.total', 'Total de eventos')}</div>
        ...
        >
          <div className="text-[10px] uppercase font-black text-blue-500/50 tracking-[0.2em] mb-4">{t('security_events.metrics.integrity_ok', 'Integridade OK')}</div>
          <div className="flex items-end justify-between">
        ...
        >
          <div className="text-[10px] uppercase font-black text-amber-500/50 tracking-[0.2em] mb-4">{t('security_events.metrics.warnings', 'Alertas')}</div>
          <div className="flex items-end justify-between">
        ...
        >
          <div className="text-[10px] uppercase font-black text-red-500/50 tracking-[0.2em] mb-4">{t('security_events.metrics.defense', 'Defesa acionada')}</div>
          <div className="flex items-end justify-between">
        ...
        <div className="relative group">
            ...
            <Card className="relative p-1.5 bg-[#0A0A15]/60 backdrop-blur-2xl border-white/5 rounded-2xl overflow-hidden shadow-2xl">
                <div className="relative flex items-center">
                    ...
                    <input
                        type="text"
                        placeholder={t('security_events.search_placeholder', 'Buscar por hash, IP ou evento...')}
                        value={search}
                    ...
                    <div className="absolute right-4 flex items-center gap-2">
                        <div className="px-3 py-1.5 rounded-lg bg-white/5 border border-white/5 text-[9px] font-black text-gray-600 uppercase tracking-widest">
                            {t('security_events.logs_found', '{{count}} logs encontrados', { count: filteredEvents.length })}
                        </div>
                    </div>
                </div>
            </Card>
        ...
        <Card noPadding className="bg-[#0A0A15]/40 backdrop-blur-3xl border-white/5 rounded-[2.5rem] overflow-hidden">
            {error ? (
              <div className="m-12 p-8 rounded-3xl bg-red-500/5 border border-red-500/10 text-red-400 flex flex-col items-center gap-4 text-center">
                ...
                <div>
                    <h3 className="font-black italic text-lg uppercase tracking-tighter text-white">{t('security_events.error_title', 'Sinal de auditoria indisponível')}</h3>
                ...
                <Button onClick={loadEvents} className="mt-4 bg-red-500/20 hover:bg-red-500/30 text-red-400 rounded-xl px-10">
                    {t('security_events.retry', 'Tentar novamente')}
                </Button>
              </div>
            ) : loading ? (
              <div className="p-32 text-center">
                ...
                <div className="mt-6 text-[10px] font-black text-gray-500 uppercase tracking-[0.3em] animate-pulse">{t('security_events.loading', 'Carregando logs de segurança...')}</div>
              </div>
              ...
              <div className="p-32 text-center">
                ...
                <h3 className="font-portal-display italic text-2xl text-white tracking-tighter uppercase font-black">{t('security_events.empty_title', 'Nenhum evento encontrado')}</h3>
                <p className="text-gray-500 text-xs font-medium uppercase mt-2 tracking-widest">{t('security_events.empty_desc', 'Nenhum desvio de segurança detectado no filtro atual.')}</p>
              </div>
            ) : (
                            ...
                            <div className="flex-1">
                                <div className="flex flex-wrap items-center gap-3 mb-2">
                                    ...
                                    <span className="flex items-center gap-1.5 text-[10px] font-bold text-gray-700 uppercase tracking-widest">
                                        <Globe className="w-3 h-3" /> {event.ip_address || t('security_events.internal_protocol', 'Evento interno')}
                                    </span>
                                </div>
                                
                                <div className="flex items-center gap-4 mb-3">
                                    <div className="flex items-center gap-2 text-[10px] font-mono text-gray-600 tracking-tighter">
                                        <Terminal className="w-3 h-3" /> {t('security_events.event_id', 'EVENTO')}: {event.id.substring(0, 8)}...
                                    </div>
                            ...
                            {event.user_id && (
                                <div className="flex items-center gap-3 bg-white/5 border border-white/5 rounded-xl px-4 py-2 group/user hover:border-primary/30 transition-all">
                                    ...
                                    <div className="text-[10px] font-bold text-gray-500 uppercase tracking-widest group-hover/user:text-white transition-colors">
                                        {t('security_events.system_user', 'Usuário do sistema')}
                                    </div>
                                </div>
                            )}
            ...
            <div className="mt-8 flex justify-center">
                <div className="flex items-center gap-8 px-10 py-4 bg-[#0A0A15]/60 backdrop-blur-xl border border-white/5 rounded-full animate-in slide-in-from-bottom duration-1000">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-[10px] font-black text-gray-600 uppercase tracking-[0.2em]">{t('security_events.buffer_ready', 'Análise pronta')}</span>
                    </div>
                    <div className="w-px h-4 bg-white/5" />
                    <div className="flex items-center gap-3">
                        <Zap className="w-4 h-4 text-primary" />
                        <span className="text-[10px] font-black text-white uppercase tracking-[0.2em]">{t('security_events.sync_enforced', 'Sincronização aplicada')}</span>
                    </div>
                </div>
            </div>
        )}
