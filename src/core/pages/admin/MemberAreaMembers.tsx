...
export const MemberAreaMembers: React.FC<MemberAreaMembersProps> = ({ area }) => {
    ...
    const getDaysOnline = (joinedAt?: string) => {
        if (!joinedAt) return 0;
        const joinedTime = new Date(joinedAt).getTime();
        if (Number.isNaN(joinedTime)) return 0;
        return Math.max(0, Math.floor((Date.now() - joinedTime) / (1000 * 60 * 60 * 24)));
    };
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 max-w-7xl mx-auto">
            ...
            >
                ...
                <div className="flex items-center gap-5 relative z-10">
                    <div>
                        <h2 className="text-xl font-black text-white italic uppercase tracking-tighter leading-none mb-1">
                            {t('members.registry.title_prefix')}{' '}
                            <span style={{ color: primaryColor }}>{t('members.registry.title_highlight')}</span>
                        </h2>
                        <div className="flex items-center gap-4">
                            <div className="flex items-center gap-2 text-white/40 text-[10px] font-bold uppercase tracking-[0.3em]">
                                <Terminal className="w-3.5 h-3.5" />
                                {t('members.registry.database_access')}
                            </div>
                            ...
                            <div className="flex items-center gap-2 text-white/60 text-[10px] font-mono uppercase tracking-[0.2em]">
                                ...
                                {t('members.registry.active_profiles', { count: members.length })}
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-3 relative z-10">
                    <button
                        ...
                        title={t('members.actions.export_database')}
                    >
                    ...
                    >
                        ...
                        <span className="text-sm">{t('members.actions.add_member')}</span>
                    </Button>
                </div>
            </div>
            ...
            <div className="flex flex-col lg:flex-row gap-3 mb-6">
                <div className="relative flex-1 group">
                    ...
                    <input
                        type="text"
                        placeholder={t('members.filters.search_placeholder')}
                        value={searchQuery}
                ...
                <div className="relative min-w-[200px] group">
                    ...
                    >
                        <option value="">{t('members.filters.status.all')}</option>
                        <option value="active">{t('members.filters.status.active')}</option>
                        <option value="suspended">{t('members.filters.status.suspended')}</option>
                        <option value="expired">{t('members.filters.status.expired')}</option>
                    </select>
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="border-b border-white/5 bg-white/[0.02]">
                                <th className="px-6 py-4 text-[10px] font-black text-white/20 uppercase tracking-[0.2em]">{t('members.table.student')}</th>
                                <th className="px-6 py-4 text-[10px] font-black text-white/20 uppercase tracking-[0.2em]">{t('members.table.status')}</th>
                                <th className="px-6 py-4 text-[10px] font-black text-white/20 uppercase tracking-[0.2em]">{t('members.table.joined_at')}</th>
                                <th className="px-6 py-4 text-right text-[10px] font-black text-white/20 uppercase tracking-[0.2em]">{t('members.table.actions')}</th>
                            </tr>
                        </thead>
                                    ...
                                    <td className="px-6 py-3">
                                        {member.status === 'active' && (
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="text-[10px] font-black uppercase italic tracking-widest text-green-500/80">{t('members.status.active')}</span>
                                            </div>
                                        )}
                                        {member.status === 'suspended' && (
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="text-[10px] font-black uppercase italic tracking-widest text-yellow-500/80 ml-2">{t('members.status.suspended')}</span>
                                            </div>
                                        )}
                                        {(member.status === 'expired' || member.status === 'revoked') && (
                                            <div className="flex items-center gap-3">
                                                <Slash className="w-4 h-4 text-red-500/60" />
                                                <span className="text-[10px] font-black uppercase italic tracking-widest text-red-500/60">{t('members.status.offline')}</span>
                                            </div>
                                        )}
                                    </td>
                                    <td className="px-6 py-3">
                                        <div className="flex flex-col">
                                            ...
                                            <span className="text-[9px] font-mono text-white/20 mt-1 uppercase tracking-[0.2em]">
                                                {t('members.days_online', { count: getDaysOnline(member.joined_at) })}
                                            </span>
                                        </div>
                                    </td>
                                    <td className="px-6 py-3 text-right">
                                        <div className="flex items-center justify-end gap-1.5 opacity-0 group-hover:opacity-100 transition-all translate-x-4 group-hover:translate-x-0 duration-500">
                                            <button
                                                ...
                                                title={t('members.actions.resend_access')}
                                            >
                                            ...
                                            <button
                                                ...
                                                title={t('members.actions.inspect')}
                                            >
                                            ...
                                            <button
                                                ...
                                                title={t('members.actions.delete')}
                                            >
                ...
                {members.length === 0 && !loading && (
                    <div className="py-16 flex flex-col items-center gap-6 bg-white/[0.01] border-t border-white/5">
                        ...
                        <div className="text-center space-y-2">
                           <p className="text-sm font-black text-white italic uppercase tracking-[0.2em]">{t('members.empty.title')}</p>
                           <p className="text-[10px] font-mono text-white/20 uppercase tracking-widest">{t('members.empty.desc')}</p>
                        </div>
                    </div>
                )}
                ...
                <div className="px-6 py-4 border-t border-white/5 bg-white/[0.01] flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-[9px] font-black text-white/30 uppercase tracking-widest">{t('members.footer.database_stable')}</span>
                        </div>
                        <div className="w-px h-3 bg-white/10" />
                        <span className="text-[9px] font-mono text-white/40 uppercase tracking-widest">
                            {t('members.footer.showing_entities', { count: members.length })}
                        </span>
                    </div>
                </div>
            </div>
            ...
            <ConfirmModal
                ...
                confirmText={t('members.delete_btn')}
                variant="danger"
                loading={isDeleting}
            />
