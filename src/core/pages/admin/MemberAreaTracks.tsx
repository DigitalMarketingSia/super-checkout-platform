        ...
        try {
            ...
            if (type === 'products') {
                ...
                items = modules.flatMap(m => m.lessons || []).map(l => ({ 
                    ...l, 
                    title: l.title,
                    subtitle: t('member_area_tracks.module_subtitle', { title: modules.find(m => m.id === l.module_id)?.title || t('member_area_tracks.unknown') })
                }));
            }
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center py-20 bg-white/[0.02] border border-white/5 rounded-[2.5rem] backdrop-blur-3xl animate-pulse">
                ...
                <p className="text-[10px] font-bold text-white/20 uppercase tracking-[0.3em] font-mono">{t('member_area_tracks.loading')}</p>
            </div>
        );
    }
        ...
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-700 max-w-7xl mx-auto">
            ...
            >
                ...
                <div className="flex items-center gap-5 relative z-10">
                    <div>
                        <h2 className="text-xl font-black text-white italic uppercase tracking-tighter leading-none mb-1">
                            {t('member_area_tracks.header.title_prefix')}{' '}
                            <span style={{ color: primaryColor }}>{t('member_area_tracks.header.title_highlight')}</span>
                        </h2>
                        <div className="flex items-center gap-3">
                            <span className="text-white/40 text-[10px] font-bold uppercase tracking-[0.3em]">{t('member_area_tracks.header.subtitle')}</span>
                            <div className="w-1 h-1 rounded-full bg-white/20" />
                            <span className="text-white/60 text-[10px] font-mono uppercase tracking-[0.2em]">{t('member_area_tracks.header.active_nodes', { count: tracks.length })}</span>
                        </div>
                    </div>
                </div>
                ...
                >
                    ...
                    <span className="text-sm">{t('member_area_tracks.actions.add_track')}</span>
                </Button>
            </div>
                ...
                <div className="mb-12 bg-white/5 border border-white/10 rounded-[2.5rem] p-8 lg:p-10 shadow-[0_30px_60px_rgba(0,0,0,0.5)] animate-in slide-in-from-top-4 duration-500 relative overflow-hidden group">
                    ...
                    <div className="relative z-10 flex flex-col lg:flex-row gap-10">
                        <div className="flex-1 space-y-10">
                            <div>
                                <label className="text-[10px] font-bold text-white/40 uppercase tracking-[0.3em] mb-4 block ml-1">{t('member_area_tracks.form.identification')}</label>
                                <input
                                    type="text"
                                    value={newTrackTitle}
                                    autoFocus
                                    ...
                                    placeholder={t('member_area_tracks.form.title_placeholder')}
                                />
                            </div>

                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div>
                                    <label className="text-[10px] font-bold text-white/40 uppercase tracking-[0.3em] mb-4 block ml-1">{t('member_area_tracks.form.content_type')}</label>
                                    <div className="grid grid-cols-2 gap-2">
                                        {[
                                            { id: 'contents', label: t('member_area_tracks.types.contents'), icon: BookOpen },
                                            { id: 'products', label: t('member_area_tracks.types.products'), icon: Zap },
                                            { id: 'modules', label: t('member_area_tracks.types.modules'), icon: Layers },
                                            { id: 'lessons', label: t('member_area_tracks.types.lessons'), icon: Play }
                                        ].map(type => (
                                            <button 
                                                key={type.id}
                                ...
                                <div>
                                    <label className="text-[10px] font-bold text-white/40 uppercase tracking-[0.3em] mb-4 block ml-1">{t('member_area_tracks.form.visual_strategy')}</label>
                                    <div className="grid grid-cols-2 gap-4 h-[calc(100%-2rem)]">
                                        ...
                                        >
                                            ...
                                            <span className="text-[10px] font-black uppercase tracking-tighter italic">{t('member_area_tracks.styles.horizontal')}</span>
                                        </button>
                                        <button 
                                        ...
                                        >
                                            ...
                                            <span className="text-[10px] font-black uppercase tracking-tighter italic">{t('member_area_tracks.styles.vertical')}</span>
                                        </button>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="lg:w-72 flex flex-col gap-3 justify-end">
                            ...
                            >
                                <span>{t('member_area_tracks.actions.create_track')}</span>
                            ...
                            >
                                {t('member_area_tracks.actions.cancel')}
                            </button>
                        </div>
                    </div>
                </div>
            ...
            <div className="space-y-12">
                                ...
                                >
                                    <div className="flex items-center gap-6">
                                        ...
                                        <div>
                                            ...
                                            <div className="flex items-center gap-4 text-[10px] font-bold text-white/20 uppercase tracking-widest">
                                                <span className="flex items-center gap-1.5"><Activity className="w-3.5 h-3.5" /> {t('member_area_tracks.track.objects_mapped', { count: track.items?.length || 0 })}</span>
                                                ...
                                                <span className="flex items-center gap-1.5 line-clamp-1 italic">
                                                    ...
                                                    {t('member_area_tracks.track.strategy', { style: t(`member_area_tracks.track.styles.${track.card_style}`) })}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="flex items-center gap-2 bg-black/40 p-2 rounded-2xl border border-white/5 shadow-inner" onClick={(e) => e.stopPropagation()}>
                                        ...
                                        <button
                                            ...
                                            title={t('member_area_tracks.actions.toggle_style')}
                                        >
                                            <Settings2 size={18} />
                                        </button>
                ...
                {tracks.length === 0 && (
                    <div className="py-32 flex flex-col items-center gap-8 bg-white/[0.02] border border-white/5 rounded-[4rem] border-dashed">
                        ...
                        <div className="text-center">
                            <h3 className="text-2xl font-black text-white italic uppercase tracking-tighter mb-2">{t('member_area_tracks.empty.title')}</h3>
                            <p className="text-white/20 text-sm font-medium max-w-xs mx-auto">{t('member_area_tracks.empty.desc')}</p>
                        </div>
                        <Button 
                            onClick={() => setIsCreating(true)} 
                        ...
                        >
                            {t('member_area_tracks.empty.cta')}
                        </Button>
                    </div>
                )}
            </div>
                    ...
                    <div className="bg-[#0A0A1F] w-full max-w-7xl h-full rounded-[4rem] border border-white/10 shadow-[0_0_150px_rgba(138,43,226,0.15)] flex flex-col overflow-hidden relative">
                        ...
                        <div className="flex-1 p-10 lg:p-14 overflow-y-auto no-scrollbar grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-8 pb-32">
                            ...
                            {filteredItems.length === 0 && (
                                <div className="col-span-full py-40 flex flex-col items-center gap-10">
                                    ...
                                    <div className="text-center">
                                        <p className="text-3xl font-black text-white italic uppercase tracking-tighter mb-4">{t('member_area_tracks.modal.no_results_title')}</p>
                                        <p className="text-white/20 text-sm font-medium uppercase tracking-widest max-w-sm mx-auto">{t('member_area_tracks.modal.no_results_desc')}</p>
                                    </div>
                                </div>
                            )}
                        </div>
                        ...
                        <div className="absolute bottom-0 inset-x-0 p-10 bg-gradient-to-t from-[#0A0A1F] via-[#0A0A1F] to-transparent flex items-center justify-between">
                            <div className="flex items-center gap-4 bg-black/40 px-6 py-4 rounded-2xl border border-white/5 backdrop-blur-3xl">
                                ...
                                <span className="text-[10px] font-mono text-white/40 uppercase tracking-[0.3em]">{t('member_area_tracks.modal.synced_nodes', { count: availableItems.length })}</span>
                            </div>
                            <div className="flex gap-2">
                                <span className="text-[10px] font-mono text-white/10 uppercase tracking-widest italic py-4">{t('member_area_tracks.modal.secure_picker')}</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <ConfirmModal
                ...
                title={t('member_area_tracks.confirm_delete.title')}
                message={t('member_area_tracks.confirm_delete.message')}
                confirmText={t('member_area_tracks.confirm_delete.confirm')}
                variant="danger"
                loading={isDeleting}
            />
        </div>
