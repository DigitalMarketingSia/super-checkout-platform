...
export const PublicTerms = () => {
    ...
    const sourceLabel = termsDocument.source === 'custom'
        ? t('legal_pages.source_custom')
        : t('legal_pages.source_default', { version: termsDocument.version });
            ...
            <header className="border-b border-white/5 bg-white/[0.02] backdrop-blur-xl sticky top-0 z-10">
                <div className="max-w-4xl mx-auto px-6 h-20 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h1 className="text-lg font-bold tracking-tight">{t('legal_pages.terms.title')}</h1>
            ...
            <main className="max-w-4xl mx-auto px-6 py-12">
                <div className="bg-[#0F0F13] border border-white/5 rounded-3xl p-8 md:p-12 shadow-2xl relative overflow-hidden">
                    ...
                    <div className="relative z-10 mb-8 rounded-2xl border border-white/5 bg-white/[0.02] p-6 text-sm text-gray-300">
                        <p><span className="font-semibold text-white">{t('legal_pages.terms.seller_label')}</span> {controllerName}</p>
                        <p><span className="font-semibold text-white">{t('legal_pages.terms.commercial_channel_label')}</span> {businessIdentity.supportEmail}</p>
                        <p><span className="font-semibold text-white">{t('legal_pages.document_source_label')}</span> {sourceLabel}</p>
                        <p><span className="font-semibold text-white">{t('legal_pages.version_label')}</span> {termsDocument.version}</p>
                        <p><span className="font-semibold text-white">{t('legal_pages.published_at_label')}</span> {formatUpdatedAt(termsDocument.publishedAt, notInformed, dateLocale)}</p>
                        ...
                        <p><span className="font-semibold text-white">{t('legal_pages.terms.hosted_checkout_label')}</span> {t('legal_pages.terms.hosted_checkout_value')}</p>
                        <p><span className="font-semibold text-white">{t('legal_pages.terms.conditional_fields_label')}</span> {t('legal_pages.terms.conditional_fields_value')}</p>
                    </div>
