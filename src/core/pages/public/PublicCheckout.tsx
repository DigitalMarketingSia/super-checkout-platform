...
type PayPalOrderPreparationResult = {
   ok: boolean;
   paypalOrderId?: string;
   message?: string;
};
   ...
   return (
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-in fade-in duration-200">
         <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-8 relative flex flex-col items-center justify-center min-h-[220px] transform transition-all animate-in zoom-in-95">
            ...
            {state === 'processing' && (
               <>
                  <h2 className="text-2xl font-bold text-gray-800 mb-6 font-['Inter']">{businessName || t('checkout.processing', 'Processando')}</h2>
                  ...
                  <h3 className="text-lg font-medium text-gray-900 text-center">{t('checkout.please_wait', 'Por favor, aguarde...')}</h3>
                  <p className="text-sm text-gray-500 text-center mt-1">{t('checkout.processing_payment', 'Estamos processando seu pagamento.')}</p>
               </>
            )}
            ...
            {state === 'error' && (
               <>
                  ...
                  <h3 className="text-xl font-medium text-[#EF4444] text-center mb-4">{t('checkout.payment_process_error', 'Não foi possível processar seu pagamento.')}</h3>
                  <div className="w-full border border-red-200 bg-red-50/50 rounded-xl p-4 text-center border-dashed">
                     <p className="text-sm text-gray-800 font-medium">{errorDetail || t('checkout.unknown_error', 'Erro desconhecido.')}</p>
                  </div>
               </>
            )}
...
const PublicCheckoutUI = ({ checkoutId: propId, stripe, elements }: { checkoutId?: string, stripe: any, elements: any }) => {
   ...
   const paypalPreparedOrderRef = useRef<PayPalPreparedOrder | null>(null);
   const paypalPreparationPromiseRef = useRef<Promise<PayPalOrderPreparationResult> | null>(null);

   const storePayPalPreparedOrder = (preparedOrder: PayPalPreparedOrder) => {
      paypalPreparedOrderRef.current = preparedOrder;
   };
   ...
   const handlePaymentMethodSelect = (method: PaymentMethod) => {
      ...
      if (method !== 'paypal') clearPayPalPreparedOrder();
   ...
   const getUpgradeIntentDisplayError = (message: string) => {
      ...
      if (normalized.includes('expired')) {
         return t('checkout.upgrade_expired', 'Este link de upgrade expirou. Gere um novo upgrade pelo portal ou pela instalação.');
      }
      if (normalized.includes('consumed') || normalized.includes('already consumed')) {
         return t('checkout.upgrade_consumed', 'Este link de upgrade já foi usado. Abra um novo upgrade para continuar.');
      }
   ...
   const validateField = (name: string, value: string) => {
      ...
      switch (name) {
         case 'name':
            if (!validateName(value)) error = t('checkout.validation.full_name', 'Digite seu nome completo (mínimo 2 palavras)');
            break;
         case 'email':
            if (!validateEmail(value)) error = t('checkout.validation.email', 'Digite um e-mail válido');
            break;
         case 'phone':
   ...
   const handleSubmit = async () => {
      ...
      if (upgradeIntentToken && !upgradeIntentContext) {
         setUpgradeIntentError(prev => prev || t('checkout.upgrade_invalid_context', 'O contexto deste upgrade não está válido para continuar.'));
         return;
      }
      ...
      if (paymentMethod === 'credit_card') {
         ...
         if (methodGateway.name === GatewayProvider.STRIPE) {
            ...
            if (!cardNumberElement) {
               showAlert(t('checkout.error_title', 'Erro'), t('checkout.card_form_not_found', 'Formulário de cartão não encontrado.'), 'error');
               setIsProcessing(false);
               return;
            }
            ...
            if (stripeError) {
               ...
               showAlert(t('checkout.card_error_title', 'Erro no cartão'), translatedError, 'error');
               setIsProcessing(false);
               return;
            }
            ...
            if (!cleanCardNumber || cleanCardNumber.length < 13) {
               showAlert(t('checkout.error_title', 'Erro'), t('checkout.card_number_error', 'Por favor, verifique o número do cartão.'), 'error');
               return;
            }

            if (!customer.expiry || !customer.expiry.includes('/')) {
               showAlert(t('checkout.error_title', 'Erro'), t('checkout.card_expiry_required', 'Por favor, preencha a validade do cartão (MM/AA).'), 'error');
               return;
            }

            const [month, year] = customer.expiry.split('/');
            if (!month || !year || parseInt(month) < 1 || parseInt(month) > 12 || year.length !== 2) {
               showAlert(t('checkout.error_title', 'Erro'), t('checkout.card_expiry_invalid', 'Data de validade inválida.'), 'error');
               return;
            }

            if (!customer.cvc || customer.cvc.length < 3) {
               showAlert(t('checkout.error_title', 'Erro'), t('checkout.card_cvc_error', 'Por favor, verifique o código de segurança (CVC).'), 'error');
               return;
            }
         }
      }

      const isPayPalPreparation = paymentMethod === 'paypal';
      if (!isPayPalPreparation) {
      ...
      try {
         ...
         debugCheckoutPayment('[PublicCheckout] Payment processing completed.', {
          ...
          if (result.success) {
            ...
            if (isPayPalPreparation) {
               showAlert('PayPal', result.message || 'Nao foi possivel preparar o pagamento PayPal. Tente novamente.', 'error');
               setProcessState('idle');
               setProcessGatewayStatus('');
               setProcessError(null);
               setIsProcessing(false);
          ...
          if (isPayPalPreparation) {
             showAlert('PayPal', 'Nao foi possivel preparar o pagamento PayPal. Tente novamente.', 'error');
             setProcessState('idle');
             setProcessGatewayStatus('');
             setProcessError(null);
             setIsProcessing(false);
   ...
   const shouldRequireCpfField = config.fields.cpf
    ...
    const handlePayPalCreateOrder = async (): Promise<PayPalOrderPreparationResult> => {
       ...
       let preparation: Promise<PayPalOrderPreparationResult>;
       preparation = (async () => {
          await handleSubmit();
          ...
          if (!preparedOrder) {
             return {
                ok: false,
                message: 'Revise os dados obrigatorios antes de continuar com o PayPal.',
             };
          }

          return { ok: true, paypalOrderId: preparedOrder.paypalOrderId };
       })();
    ...
    const handlePayPalApproval = async (providerOrderId: string) => {
       const preparedOrder = paypalPreparedOrderRef.current;
       if (!preparedOrder || !paypalGateway || !data) {
               ...
               >
                  <div className="max-w-2xl mx-auto px-4 flex items-center justify-between">
                     <div className="flex items-center gap-2 text-sm font-medium opacity-90">
                        ...
                        <span className="hidden sm:inline">{t('checkout.offer_expires_in', 'A oferta expira em:')}</span>
                        <span className="sm:hidden">{t('checkout.expires_in', 'Expira em:')}</span>
                     </div>
            ...
            <div className={`max-w-2xl mx-auto px-4 space-y-6 ${config.timer?.active ? 'mt-3' : 'mt-6'}`}>
               ...
               {config.header_image && (
                  <div className={`w-full rounded-2xl overflow-hidden shadow-sm ${!config.timer?.active ? 'mt-4' : ''}`}>
                     <img src={config.header_image} alt={t('checkout.header_image_alt', 'Imagem de destaque do checkout')} className="w-full h-auto block" />
                  </div>
               )}
                  ...
                  <div className="w-20 h-20 flex-shrink-0 bg-gray-100 rounded-lg overflow-hidden">
                   ...
                   <div className="flex-1">
                      ...
                      {data.product.price_fake && (
                         <p className="text-xs text-gray-500 mt-1">{t('checkout.from_price', 'De')} <span className="line-through">{formatCurrency(data.product.price_fake)}</span> {t('checkout.only_for', 'por apenas')}</p>
                      )}
                     ...
                     <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                           <p className={`text-[11px] font-black uppercase tracking-[0.24em] ${upgradeIntentError ? 'text-red-600' : 'text-emerald-700'}`}>
                              {t('checkout.upgrade_link', 'Vínculo de upgrade')}
                           </p>
                           {upgradeIntentLoading ? (
                              <p className="mt-2 text-sm text-gray-600">
                                 {t('checkout.validating_upgrade_account', 'Validando qual conta deve receber este upgrade...')}
                              </p>
                           ) : upgradeIntentError ? (
                              ...
                              <>
                                 <p className="mt-2 text-sm text-gray-700">
                                    {t('checkout.upgrade_applies_to', 'Este upgrade será aplicado automaticamente para:')}
                                 </p>
                                 <p className="mt-2 text-base font-bold text-gray-900">
                                    {upgradeIntentContext?.beneficiary.display_name || t('checkout.current_account', 'Conta atual')}
                                 </p>
               ...
               <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 space-y-4">
                  <h3 className="font-bold text-gray-900 flex items-center gap-2">
                     ...
                     {t('checkout.personal_data', 'Dados pessoais')}
                  </h3>

                  {config.fields.name && renderInput('name', t('checkout.fields.full_name', 'Nome completo'))}
               ...
               <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                   ...
                   <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-6">
                     {canPayWithCreditCard && (
                        ...
                        >
                           ...
                           <span className="text-sm font-bold">{t('checkout.credit_card', 'Cartão de crédito')}</span>
                        </button>
                     )}
                     ...
                     {canPayWithPix && (
                      ...
                      {canPayWithPayPal && (
                         ...
                         >
                            ...
                            <img src="/paypal-dark.svg" alt="PayPal" className="h-5 w-auto max-w-[72px] object-contain" />
                         </button>
                      )}
                  ...
                  {paymentMethod === 'pix' && (
                     <div className="bg-green-50 border border-green-200 rounded-xl p-4 animate-in fade-in duration-300">
                        <div className="flex items-center gap-2 text-green-800 font-bold mb-3">
                           <PixIcon className="w-5 h-5" /> {t('checkout.pay_with_pix', 'Pague com Pix')}
                        </div>
                        <p className="text-sm text-green-900">{t('checkout.pix_immediate_access', 'Liberação imediata do acesso após o pagamento.')}</p>
                     </div>
                  )}
                  ...
                  {paymentMethod === 'boleto' && (
                     <div className="bg-gray-50 border border-gray-200 rounded-xl p-4 animate-in fade-in duration-300">
                        <div className="flex items-center gap-2 text-gray-800 font-bold mb-2">
                           <Barcode className="w-5 h-5" /> {t('checkout.boleto_info', 'Informações do boleto')}
                        </div>
                        <p className="text-sm text-gray-600">{t('checkout.boleto_due', 'Vencimento em 2 dias úteis.')}</p>
                     </div>
                  )}
                  ...
                  {!paymentMethod && (
                     <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 flex items-center gap-3 text-yellow-800 text-sm animate-pulse">
                        <AlertCircle className="w-5 h-5" />
                        {t('checkout.choose_payment_method', 'Escolha uma forma de pagamento.')}
                     </div>
                  )}
                  ...
                  <div className="space-y-4 animate-in zoom-in-95 duration-300">
                     ...
                     <div className="bg-gradient-to-r from-yellow-400 to-orange-500 px-4 py-2 rounded-xl shadow-sm">
                        <h3 className="text-xs font-bold text-white uppercase tracking-wide text-center flex items-center justify-center gap-2">
                           {t('checkout.bump_header', 'Oportunidade especial agora')}
                        </h3>
                     </div>
               ...
               {/* RESUMO DO PEDIDO */}
               <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
                  <h3 className="font-bold text-gray-900 mb-4">{t('checkout.summary', 'Resumo')}</h3>
                  ...
                  <div className="text-center py-6 border-t border-gray-100 mt-8">
                   <p className="text-[10px] text-gray-400 font-bold flex items-center justify-center gap-1 mb-1 uppercase tracking-widest">
                      <ShieldCheck className="w-4 h-4 text-[#10B981]" /> {t('checkout.secure_purchase', 'Compra 100% segura')}
                   </p>
      ...
      const handlePaymentMethod = async (ev: any) => {
         try {
            ...
            if (result.success) {
               ...
               ev.complete('fail');
               showAlert(t('checkout.error_title', 'Erro'), result.message || t('checkout.wallet_payment_failed', 'Falha no pagamento via carteira.'), 'error');
            }
         } catch (err) {
   ...
   if (simulated) {
      return (
         <div className="w-full rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-center">
            <p className="text-xs font-bold uppercase tracking-wide text-amber-700">{t('checkout.simulated_mode', 'Modo simulado')}</p>
            <p className="mt-1 text-sm font-medium text-amber-900">
               {t('checkout.wallets_localhost_only', 'Carteiras exibidas apenas para validação visual em localhost.')}
            </p>
         </div>
      );
   }
...
const PayPalCheckoutButton = ({
   ...
   clientId: string;
   currency: string;
   onCreateOrder: () => Promise<PayPalOrderPreparationResult>;
   ...
   }, [onCreateOrder, onApprove, onCancel, onError]);
   ...
   useEffect(() => {
         ...
         buttonsInstance = paypal.Buttons({
            ...
            createOrder: async () => {
               ...
               if (!result.ok || !result.paypalOrderId) {
                  ...
                  throw new Error('PAYPAL_PREPARATION_REJECTED');
               }
            ...
            onError: (error: unknown) => {
               ...
               if (suppressPreparationErrorRef.current || message.includes('PAYPAL_PREPARATION_REJECTED')) {
      ...
      if ((window as any).paypal?.Buttons) {
         ...
         existingScript.addEventListener('error', () => {
            if (!disposed) {
               setState('error');
               callbacksRef.current.onError('Nao foi possivel carregar o PayPal. Verifique sua conexao e tente novamente.');
            }
         }, { once: true });
      } else if (!normalizedClientId) {
         setState('error');
         callbacksRef.current.onError('O PayPal nao esta configurado para este checkout.');
      } else {
