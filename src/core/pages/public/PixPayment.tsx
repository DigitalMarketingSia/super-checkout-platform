      ...
      <header className="bg-white border-b border-gray-200 py-4">
        <div className="max-w-4xl mx-auto px-4 flex items-center justify-between">
          <div className="flex items-center gap-2 text-primary font-bold text-xl">
            ...
            <span>{t('pix.secure_payment', 'Pagamento seguro')}</span>
          </div>
          <div className="text-xs text-gray-500 hidden sm:block">
            {t('pix.order_id', 'ID do pedido')}: <span className="font-mono font-medium text-gray-900">{orderId}</span>
          </div>
        </div>
      </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="md:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-green-100 overflow-hidden">
              <div className="bg-green-50 p-4 border-b border-green-100 flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-lg font-bold text-gray-900">{t('pix.generated_title', 'Pix gerado com sucesso!')}</h1>
                  <p className="text-xs text-gray-500">{t('pix.generated_desc', 'Realize o pagamento para liberar seu acesso imediatamente.')}</p>
                </div>
              </div>

              <div className="p-6 flex flex-col items-center justify-center text-center">
                <p className="text-sm text-gray-500 mb-4">{t('pix.scan_qr', 'Escaneie o QR Code abaixo no app do seu banco:')}</p>
                ...
                <div className="w-full max-w-md space-y-3">
                  <div className="flex items-center justify-between text-xs text-gray-500 px-1">
                    <span>{t('pix.copy_paste', 'Pix Copia e Cola')}</span>
                    <span className="flex items-center gap-1 text-orange-500 font-medium">
                      <Clock className="w-3 h-3" />
                      {t('pix.expires_in', 'Expira em {{time}}', { time: `${timeLeft.minutes}:${timeLeft.seconds.toString().padStart(2, '0')}` })}
                    </span>
                  </div>
                  ...
                  <div className="relative group">
                    ...
                    >
                      ...
                      {copied ? t('pix.copied', 'Copiado!') : t('pix.copy', 'Copiar')}
                    </button>
                  </div>

                  <p className="text-[10px] text-gray-400 mt-4">
                    {t('pix.approval_hint', 'Após o pagamento, a aprovação ocorre em instantes e você receberá os dados de acesso no e-mail {{email}}.', { email: orderData?.customer?.email })}
                  </p>
                </div>
              </div>
              ...
              <div className="bg-gray-50 p-4 border-t border-gray-100 flex items-center justify-center gap-3">
                ...
                <span className="text-sm text-gray-500 font-medium">
                  {paymentConfirmed ? t('pix.confirmed_redirecting', 'Pagamento confirmado. Redirecionando...') : t('pix.waiting_bank', 'Aguardando confirmação do banco...')}
                </span>
              </div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h3 className="text-sm font-bold text-gray-900">{t('pix.how_to_pay', 'Como pagar?')}</h3>
                  <ol className="text-xs text-gray-500 list-decimal list-inside mt-2 space-y-1">
                    <li>{t('pix.steps.open_bank', 'Abra o aplicativo do seu banco.')}</li>
                    <li>{t('pix.steps.choose_pix', 'Escolha a opção Pix.')}</li>
                    <li>{t('pix.steps.scan_or_copy', 'Selecione Ler QR Code ou Pix Copia e Cola.')}</li>
                    <li>{t('pix.steps.confirm', 'Confira os dados e confirme o pagamento.')}</li>
                  </ol>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              <h3 className="font-bold text-gray-900 mb-4 pb-4 border-b border-gray-100">{t('pix.summary_title', 'Resumo da compra')}</h3>
              ...
              <div className="mt-6 pt-4 border-t border-gray-100">
                <div className="flex justify-between items-center">
                  <span className="text-gray-500 text-sm">{t('pix.total_to_pay', 'Total a pagar')}</span>
            ...
            <div className="text-center space-y-2">
              <p className="text-xs text-gray-400">{t('pix.sold_by', 'Vendido e entregue por')}</p>
