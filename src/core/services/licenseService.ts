...
const normalizeLicensePayload = (payload: any): License | null => {
    if (!payload || typeof payload !== 'object') return null;

    return {
        ...payload,
        key: payload.key || payload.license_key || '',
        max_instances: payload.max_instances ?? payload.max_installations ?? 0,
    } as License;
};
