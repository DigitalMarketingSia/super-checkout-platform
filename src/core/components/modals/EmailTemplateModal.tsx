...
export const EmailTemplateModal: React.FC<EmailTemplateModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm animate-in fade-in">
      <div className="flex h-[95vh] w-full max-w-4xl flex-col rounded-[2.5rem] border border-white/10 bg-[#0C0C14] shadow-2xl relative overflow-hidden animate-in zoom-in duration-300">
        ...
        <div className="flex items-center justify-between border-b border-white/5 p-6 relative z-10">
          <div>
            <h2 className="text-xl font-portal-display italic uppercase text-white leading-none mb-1.5">{t('email_template_modal.title')}</h2>
            <p className="text-xs text-gray-400 font-mono tracking-wider">{template.name}</p>
          </div>

          {/* View Modes Segments Selector */}
          <div className="mr-4 flex items-center gap-1 rounded-2xl border border-white/10 bg-black/20 p-1">
            <button
              onClick={() => setViewMode('visual')}
              className={`flex items-center gap-1.5 rounded-xl px-4 py-2 text-xs font-bold transition-all duration-300 ${
        ...
        <div className="custom-scrollbar flex-1 space-y-6 overflow-y-auto p-6 relative z-10">

          {/* Variable Insert Box */}
          <div className="rounded-2xl border border-primary/20 bg-[#07070F] p-5 shadow-inner">
          ...
          {isPurchaseConfirmationTemplate && (
            <div className="rounded-2xl border border-white/10 bg-black/25 p-5 text-gray-300 text-xs leading-relaxed font-sans font-medium">
          ...
          <div className="space-y-6">
            ...
            {viewMode === 'visual' && (
              <div className="animate-in space-y-2 fade-in duration-300">
                ...
                <div className="min-h-[400px] overflow-hidden rounded-2xl bg-white text-gray-900 border border-white/10 shadow-lg p-2">
                  <Editor
                    value={body}
              ...
              <div className="animate-in space-y-2 fade-in duration-300">
                ...
                <div className="group/editor relative">
                  <textarea
                    ...
                    value={body}
                    onChange={(event) => setBody(event.target.value)}
                    className="h-96 w-full resize-none rounded-2xl border border-white/[0.12] bg-[#07070F] px-4 py-3.5 font-mono text-xs leading-relaxed text-gray-200 shadow-inner outline-none transition-all focus:border-primary/50 focus:ring-1 focus:ring-primary/50 focus:shadow-[0_0_15px_rgba(138,43,226,0.1)]"
                    spellCheck={false}
        ...
        <div className="flex justify-end gap-3 border-t border-white/5 bg-black/45 p-6 relative z-10">
          <Button variant="ghost" onClick={onClose} disabled={loading} className="text-gray-400 hover:text-white rounded-xl h-11 px-5 text-xs font-bold transition-all">
            {t('common.cancel')}
          </Button>
          <Button
            onClick={handleSave}
            disabled={loading}
            className="bg-primary hover:bg-primary-hover text-white rounded-xl h-11 px-6 text-xs font-bold shadow-[0_4px_16px_rgba(138,43,226,0.35)] transition-all duration-300 flex items-center gap-2"
          >
