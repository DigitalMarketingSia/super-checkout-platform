...
export const ProductSalesModal: React.FC<ProductSalesModalProps> = ({ isOpen, onClose, product }) => {
    ...
    const handleBuy = async () => {
        ...
        if (product.redirect_link) {
            ...
            alert(t('sales_modal.no_purchase_link', 'Não há link de compra configurado para este produto. Contate o administrador.'));
        }
    };
                    ...
                    <div className="p-6 md:p-8 flex flex-col">
                        <div className="mb-6">
                            <span className="inline-block px-3 py-1 rounded-full bg-red-500/10 text-red-500 text-xs font-bold mb-3 border border-red-500/20">
                                {t('sales_modal.special_offer', 'Oferta especial')}
                            </span>
                            ...
                            <p className="text-gray-400 text-sm leading-relaxed line-clamp-1 md:line-clamp-4">
                                {product.description || t('sales_modal.default_description', 'Aproveite esta oportunidade exclusiva para adquirir este produto e desbloquear todo o seu potencial.')}
                            </p>
                        </div>
                        ...
                        <div className="mt-auto space-y-6">
                            ...
                            <div>
                                {product.price_fake && product.price_fake > 0 && (
                                    <div className="text-gray-500 text-sm line-through mb-1">
                                        {t('sales_modal.from_price', 'de')} {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(product.price_fake)}
                                    </div>
                                )}
                                <div className="flex items-baseline gap-2">
                                    <span className="text-sm text-gray-400">{t('sales_modal.only_for', 'por apenas')}</span>
                            ...
                            >
                                ...
                                {t('sales_modal.buy_now', 'Comprar agora')}
                            </button>

                            <div className="flex items-center justify-center gap-4 text-xs text-gray-500">
                                <span className="flex items-center gap-1"><Check className="w-3 h-3 text-green-500" /> {t('sales_modal.secure_payment', 'Pagamento seguro')}</span>
                                <span className="flex items-center gap-1"><Check className="w-3 h-3 text-green-500" /> {t('sales_modal.immediate_access', 'Acesso imediato')}</span>
                            </div>
                        </div>
                    </div>
                </div>
