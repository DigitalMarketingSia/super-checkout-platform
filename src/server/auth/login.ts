...
function getCentralSupabaseAnonKey(): string {
...
function getLocalSupabaseServiceKey(): string {
    return process.env.SUPABASE_SECRET_KEY
        || process.env.SUPABASE_SERVICE_ROLE_KEY
        || '';
}

function isLegacyApiKeyDisabledError(error: any): boolean {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    if (limited) {
        ...
        await logAuthEvent({
            ...
            serviceKey: getLocalSupabaseServiceKey(),
    ...
    if (target === 'central') {
        ...
        supabaseAnonKey = getCentralSupabaseAnonKey();
    } else {
        // Default: Local Supabase (Login.tsx, MemberLogin.tsx)
        supabaseUrl = process.env.VITE_SUPABASE_URL || process.env.NEXT_PUBLIC_SUPABASE_URL || '';
        supabaseAnonKey = getLocalSupabaseAnonKey();
    }
            ...
            try {
                ...
                if (profile?.totp_enabled) {
                    if (!profile.totp_secret_encrypted) {
                        ...
                        return res.status(500).json({
                            error: '2FA esta habilitada, mas a configuracao local esta incompleta.',
                ...
                console.error(`[Auth/Login] 2FA inspection failed closed for ${maskedEmail}:`, inspectionError?.message || inspectionError);
                await logAuthEvent({
                    supabaseUrl: auditSupabaseUrl,
                    serviceKey,
                    eventType: 'two_factor_inspection_failed',
                    severity: 'CRITICAL',
                    ip,
                    userAgent: getUserAgent(req),
                    ...
                    metadata: {
                        ...
                        email: maskedEmail,
                        source: 'auth_login_proxy',
                        reason: inspectionError?.message || String(inspectionError),
                        two_factor_required_unknown: true
                    }
                });

                return res.status(500).json({
                    error: 'Nao foi possivel validar as exigencias de 2FA.',
                    error_code: 'two_factor_inspection_failed'
                });
            }
        }
